...
| `RTA-004.5.2` 补 aggregate `xcrun` 入口与 CLI 级 cross-backend compile summary 断言矩阵 | DOING | aggregate `xcrun` compile summary 与 `mtl-device` 在 conflict / partial / unavailable / override 上都有更直接的端到端断言 | 已补 `compile_aggregate_source(...)` 的 `xcrun` summary mock matrix，并在 `xcrun` / `mtl-device` CLI 上补 aligned-disable 端到端 summary 断言；后续继续扩到更多 CLI reason-code 场景 |
