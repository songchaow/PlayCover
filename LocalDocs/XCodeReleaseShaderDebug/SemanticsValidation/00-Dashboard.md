...
- 本轮继续下钻 `4bfca3ce...` 后，证据链进一步收敛成更窄的 compare residual：`entry / resource / builtin` 语义保持一致，`compile-summary.json` 继续显示 `originalFastMathMode=enable`、`effectiveMetalArgs=["-ffast-math"]`，CFG 完全一致（`basicBlockCount 55 / br 36 / condbr 18 / phi 18 / select 9`），AIR intrinsic 统计一致，剩余主因是 AIR 后端 GEP lowering 把部分 arithmetic 展开成更多显式 GEP（memory +16, arithmetic -9, vector +5, totalAbsoluteDelta 30），以及模块级 `targetTriple` 从 `air64_v24-apple-ios15.0.0` 变为 `air64-apple-macosx15.0.0` 的 round-trip 平台差异。
...
| 任务 | 状态 | 结束标准 | 详细文档 |
|---|---|---|---|
| `CC-001` ~ `CC-003` 主线建立与首轮高频 residual 收敛 | DONE | 已完成 canonical-diff 驱动主线切换，并把主线风险收敛到 `L3 = 0` 的稳定状态 | 本文档 / `04-L2-CanonicalCompareAndRiskGrading.md` / `difference-analysis/` |
| `CC-003.1` ~ `CC-003.9` 参数 / metadata / resource / return wrapper / constant buffer / 输出语义 / fast-math family 收敛 | DONE | 参数与资源语义保真、single-field return wrapper、constant buffer dereferenceable、`[[position, invariant]]` 与 fast-math compile posture 等首轮高价值语义缺口已完成闭环 | `difference-analysis/resource-metadata-addrspace/` / `difference-analysis/resource-type-name-preservation/` / `difference-analysis/single-field-return-wrapper/` / `difference-analysis/constant-struct-reference-dereferenceable/` / `difference-analysis/position-invariant-output/` / `difference-analysis/fast-math-compile-posture/` |
| `CC-003.10` ~ `CC-003.17` structured CFG / stage-in / discard / shared-CFG shape drift / materialization family 收敛 | DONE | mixed CFG structured emission、fragment `stage_in` 语义、`discard_fragment` lowering，以及多轮 shared-CFG / scalar-vector materialization compare residual 已完成首轮系统收敛 | `difference-analysis/mixed-cfg-structured-emission/` / `difference-analysis/fragment-stage-in-semantics-preservation/` / `difference-analysis/fragment-discard-lowering/` / `difference-analysis/shared-cfg-shape-drift-normalization/` / `difference-analysis/scalar-vector-materialization-normalization/` / `difference-analysis/scalar-vector-cast-materialization-normalization/` |
| `CC-003.18` ~ `CC-003.25` shared-CFG split/merge 与窄 materialization residual 持续降噪 | DONE | corpus / diagnostics 中多轮 shared-CFG split-merge、vector/scalar cast 与 select/materialization residual 已持续下调，主线保持 `L3 = 0` 并稳定压低 `L2` | `difference-analysis/shared-cfg-split-merge-normalization/` / `difference-analysis/scalar-vector-cast-materialization-normalization/` |
...
- **`targetTriple` 的 ios → macosx 变化是 round-trip 平台差异，不是实现缺口。** 当 original IR 的 `target triple` 为 `air64_v24-apple-ios*` 而 regenerated 为 `air64-apple-macosx*` 时，这是 MSL 在 macOS SDK 上重编译的必然结果，converter 无法控制；所有 corpus L2 样本都带有这个差异，应通过 compare 归一化标记为 L0。
