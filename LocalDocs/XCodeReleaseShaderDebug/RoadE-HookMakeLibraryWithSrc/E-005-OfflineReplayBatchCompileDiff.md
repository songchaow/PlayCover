...
当前日常不再依赖这类旧的数量型 baseline 来判断 `E-005b` 是否成立；相关历史统计已下沉到 `00-Dashboard-Archive.md`。当前仍有决策价值的是：

- 已可对 `test-data` 与 `ShaderCorpus` 统一执行 replay + batch compile + preflight / failure clustering
- `FORCE_PLAYTOOLS_REBUILD=1 ./BuildScripts/sync_playtools_xcframework.sh` 继续作为标准构建守门路径
- 更细的 run 结果、样本数量与阶段性统计，以 dashboard 的“最新基线”或 archive 为准

这说明 **`E-005b` 已经不只是“能批量跑 metal”，而是能稳定作为当前日常自动化 compile / preflight / 聚类守门路径**。
