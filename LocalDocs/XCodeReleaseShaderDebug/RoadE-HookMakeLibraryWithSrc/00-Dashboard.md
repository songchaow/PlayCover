...
| 当前 `E-006d8` 第一层矩阵结论（2026-04-06） | `off1/off2/on1/on2/on3/on4/on5/on6` 八轮快照继续证明：**同模式输入稳定**、共享 `moduleKey` 为 **91/91**、单模块 `.bc/.ll/.metal` 本体未出现语义级随机漂移；将 fresh `replacement-on-run6` 纳入后，`build/e006d-run-matrix.json` 最新摘要为：`mode=off allInputStable=True`、`mode=on allInputStable=True`、`mode=on allReplacementAttemptStable=False`、`mode=on missingAttemptWhileEnabledPairs=9`、`off-vs-on allPairsDifferent=False`、`differentPairCount=10` |
...
| 当前 replacement 证据基线（2026-04-06） | 标准 `FORCE_PLAYTOOLS_REBUILD=1 ./BuildScripts/sync_playtools_xcframework.sh` + `./BuildScripts/build_and_install.sh` + fresh `replacement-on-run6` 已进一步把 live 证据往前推：`create_session(timeout=30)` 再次成功得到 `session=ready`，`RuntimeLaunchDiagnostics` 继续命中完整 launch / listener / registration 链路；`replacement-on-run6` 快照摘要为 `manifestLines=2104`、`modules=91`、`replacements=179`、`diagnostics=18`、`gputraceMSL=n/a`。更关键的是，本轮修改 `LLVMDisassembler` 后，最新 manifest 增量里的 failure 已前移为 `Host bridge reported an error: Session not registered: <sessionId>`，而不再是本地 spawn 触发的 `Failed to launch llvm-dis: Operation not permitted` |
| 当前 blocker 与下一步（2026-04-06） | 真正的 blocker 已从“fresh 注入后 runtime 可能根本没进 `PlayCover.launch()`”继续前移为四件事：1）host 侧已补上 stale-session cleanup 时主动断开 registration channel 的修复，下一步要带着该修复重新做 fresh on-run，确认 `Session not registered` 是否从“host/runtime split-brain”进一步收敛为更窄的 reachability 问题；2）`capture_metal_frame` 使用自定义工作区 `output_path` 会因权限失败，只能先导出到目标 app 容器默认 `Captures/` 后再纳入标准快照；3）上一轮成功 `.gputrace` 仍只有 `2/11` 个合法 MSL，说明 live/capture 链路恢复后，最终 trace 侧源码覆盖仍偏低；4）还没有对代表性 `replacement-off-run1` vs `replacement-on-run5` 做正式的 draw call / Render Encoder / Pipeline State 结构化 diff。下一步应优先沿这四条自动化路径继续收敛，而不是再把重心放回“入口有没有跑到”或重新引入人工 gate |
