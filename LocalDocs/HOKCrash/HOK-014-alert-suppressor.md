...
>
> **阅读建议**：修改 HOK-014 / HOK-010 相关代码时总是读取；HOK-016 闭合前
> HOK-014 仍是"用户看不到错误对话框"的硬约束，不能轻易动。
>
> **相关文档**：
>
...
验证前置条件、基线 live 命令与判据、bundle gate 回归检查、候选 E
re-apply 安全性验证一律沉淀到 `HOK-014-appendix-verification.md`。
概要：

- **基线 live**：跑 `hok006_ngr_lldb_runner.py` 60s settle，确认
  `lldbStopObserved=False`、`hok013_ngr_slot_preheat status=primed`、
  `hok014_ngr_alert_suppressor_installed` 与至少 1 次
  `hok014_ngr_alert_suppressed` 事件、无新 `NGR-*.ips`。
- **bundle gate 回归**：`grep -l "hok014_ngr" RuntimeLaunchDiagnostics/
  */launch-events.jsonl` 应只命中 `com.tencent.ngr` 子目录。
- **`blockingDialogs>=1` 不等于回归**：必须结合 window 尺寸 +
  `ownerName` 来区分 sheet（260×204 典型）vs NGR 主游戏窗口
  （≥1024×512）。

> **何时进附录**：要真正跑一轮 HOK-014 验证、或怀疑 alert 没被压制 /
> bundle gate 被溢出时读；日常主文档浏览不必进入。
...
- `LocalDocs/HOKCrash/HOK-014-appendix-verification.md`：live 验证前置
  条件 / 基线命令 / bundle gate 回归 / 常见误区（`blockingDialogs>=1`
  判读、HOK-010 与 HOK-014 的职责区分、swizzle 白名单思路）。
- `LocalDocs/HOKCrash/HOK-013-slot-preheat.md`：HOK-013 的实现口径、
  bundle gate 的复用入口、stub object 布局。
- `LocalDocs/HOKCrash/HOK-012-工具链与方法论归档.md`：b.0 对话框
