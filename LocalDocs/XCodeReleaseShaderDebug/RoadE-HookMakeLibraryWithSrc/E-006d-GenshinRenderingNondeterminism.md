...
- 2026-04-06 还补齐了 `E-006d8b1`：新增 `Scripts/e006d_render_diff.py`，把 `build/e006d-run-snapshots/<label>/<bundle-id>` 标准快照正式接到 `xcode_gpu_ops.py`、`collect_cbs.py`、`collect_re_details.py`。当前这条线已具备统一入口：agent 可直接按 run label 自动解析快照中的 `.gputrace`，导出 `frame_dump/`、`cb_data.json`、可选 `key_pass_details.json`，并自动汇总 `comparison.json + summary.txt`。因此绘制内容差异主线当前剩余的 blocker 已进一步收敛为“在 GUI 自动化前提满足时，正式对代表性 off/on run 跑出第一份结构化 diff”。
...
- 截至当前，**绘制内容差异**这条线的“入口脚本缺失”已经关闭：`Scripts/e006d_render_diff.py` 已把 run label 解析、`.gputrace` 定位、`frame_dump` / `cb_data.json` / `key_pass_details.json` 采集，以及 `comparison.json + summary.txt` 汇总接成统一 runner。当前缺的已不再是“怎么调用这些脚本”，而是“在具备 Xcode GUI / Accessibility / `cliclick` 条件时，正式用该入口跑出 `replacement-off-run1` vs `replacement-on-run5` 的首份结果并回写结论”。
...
5. `Scripts/analyze_capture_run_matrix.py` 与 `Scripts/compare_capture_runs.py` 的现有结论已经足够回答：同模式输入稳定、fresh build/install 已恢复 replacement 证据、live session/capture 主链也已恢复，但跨模式稳定不同仍未成立；因此当前**不能只继续看 shader / corpus / trace 哈希**，而应把下一步明确切到 `replacement-off-run1` vs `replacement-on-run5` 的绘制内容结构化 diff。当前推荐直接使用：`python3 Scripts/e006d_render_diff.py collect-pair --bundle-id com.miHoYo.Yuanshen --run-a replacement-off-run1 --run-b replacement-on-run5`，默认先落 `frame_dump` + `cb_data.json` + `comparison.json`，必要时再追加 `--include-re-details` 补齐 `key_pass_details.json`
