...
| 3 | **trace 合法 MSL 覆盖偏低** | **进行中（当前最高优先级）** | preload 修复已生效：`on-run11` 从 `visibleMSL=0` 提升到 `visibleMSL=3`（`922 refs = 3 valid + 9 non-MSL + 910 missing`），且 3 个可见 MSL 已归因到 replacement 聚合源码。经当前 canonical 口径与新 raw-index 工具重算，`off-run1` 为 `899 refs = 2 referenced valid + 9 non-MSL + 888 missing`，`on-run11` 为 `922 refs = 3 referenced valid + 9 non-MSL + 910 missing`；missing 集合拆解为 `shared=583 / only off=305 / only on=327 / net=+22`。同时两边 raw index 都各含 `54` 个 `14/15` 位 short token（shared `37`，only off `17`，only on `17`），`on-run11` 只是额外把其中 `2` 个落盘成可见短 hash bplist。下一步：解释 raw short token → bundle 可见文件的写入条件，并确认这与 canonical missing 的关系 |
...
| E-006d8 b3b raw short-hash 拆解（2026-04-06 夜） | 更新后的 `check_gputrace_sources.py` / `compare_capture_runs.py` 证明：`off-run1` raw index 长度分布为 `14=4,15=50,16=899`，`on-run11` 为 `14=5,15=49,16=922`；两边 raw short token **都**是 `54` 个（shared `37`，only off `17`，only on `17`）。因此 `14/15` 位 short token 现象不是 replacement 独有；当前待解释的是“哪些 raw short token 会额外落盘成 bundle 可见文件”，以及这与 `shared=583 / only off=305 / only on=327` 的 canonical missing 集合之间的关系 |
...
| E-006d8-b3b | ↳ 对照 E-006c / `off-run1` 可见 trace 排查 missing 的 bundle 写入条件 | **TODO（当前最高优先级；已完成基线校正 + raw short-hash/index 拆解工具）** | [E-006d](E-006d-GenshinRenderingNondeterminism.md) |
