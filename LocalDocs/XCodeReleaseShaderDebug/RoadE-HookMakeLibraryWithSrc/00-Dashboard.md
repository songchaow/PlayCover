...
- **当前最该做的事**：`E-006d8` 的第一轮 run matrix 已经够用，下一步**不是**继续机械补 run，也不是回头扩 lowering；而是把 fresh `replacement=on` 的自动化 live 链路重新稳定下来。本轮已先对 `LLVMDisassembler` 做 host bridge **重试 + 防误回退到本地 `posix_spawn`** 的收口，`replacement-on-run4` 快照摘要与 `on3` 保持一致（`manifestLines=1636`、`modules=91`、`replacements=36`、`diagnostics=18`），说明 artifact / replacement 基线未被新改动打坏；但 live blocker 仍未解除，而且最新形态已从“`ready` 后掉 session”前移为“**app 进程存活但 `create_session(timeout=120)` 仍未等到 runtime 注册，`list_sessions` 为空**”。与此同时，`55` 次失败 attempt 仍集中落为 `reasonCode=exception`，`detail="Failed to launch llvm-dis: Operation not permitted"`。在这两条线收敛前，不应继续扩 run、也不应把日常 gate 推回人工协助；只有先在 agent 可独立完成的流程内恢复**至少一轮 `replacement=on` 且带 `.gputrace + aggregate source` 的稳定样本**，矩阵才有资格继续回答“替换 vs 不替换”是否稳定不同。
