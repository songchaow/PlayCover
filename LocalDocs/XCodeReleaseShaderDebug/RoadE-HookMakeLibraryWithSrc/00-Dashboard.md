...
- **`E-006g`（当前最高优先级）**：解决 `恋与深空` 在同时启用 `metal capture + startup injection + shader replacement` 时的启动崩溃。**`791A306ED1B6648B_4577 / ec0c6f...`、`F474... / bbb32d...`、`A101... / 82d1...`、`45AE24662B56C487_14497 / 1cdc9318...`、`8ABA7F7B315002A3_11361 / 29b821c6...` 与 `D4CAEB2BF7815C4F_6353 / f567fbc2...` 已先后退出 latest fresh blocker；本轮先用 failure-path 样本补齐 `read_texture_2d` 的有符号坐标到 `uintN(...)` lowering，并新增最小回归样本 `test_read_texture_2d_int_coords.{ll,metal}`，随后按标准脚本完成 `build_and_install + fresh case E` 复测。最新一轮 `processLaunchId=launch-15751-41e75001-ec5e-4b60-be84-659e600fe989` 仍会在 settle window 后落到 `replacement_compile_failed`，但 actionable blocker 已前移到 `4010578BBE3B30E1_4673 / e3c0894b45454fb3dbdb7d07523ed6fd2645c3e6b8939b11c02d3709f70df905`，compiler message 主体为 `mtl_FrontFace` 未声明。当前默认入口已切换为围绕该 failure-path 样本做离线 replay / 最小样本化 / builtin lowering 修复。** 详细路径见 `E-006g-LoveAndDeepspaceTripleToggleStartupCrash.md`。
...
| `恋与深空` 三开关启动兼容性 blocker（2026-04-08） | `B/C` 可在 launch 后继续存活，`D/E` 会在 `playcover_launch_complete` 之后约 7~8 秒内 late crash。`791A... / ec0c6f...`、`F474... / bbb32d...`、`A101... / 82d1...`、`45AE24662B56C487_14497 / 1cdc9318...`、`8ABA7F7B315002A3_11361 / 29b821c6...` 与 `D4CAEB2BF7815C4F_6353 / f567fbc2...` 已先后退出 latest compile blocker；本轮先以 failure-path 样本完成 `read_texture_2d` 的 coord lowering 修复，把 `<N x i32>` 坐标显式转成 `uintN(...)`，并新增最小回归样本 `test_read_texture_2d_int_coords.{ll,metal}`，随后完成 `f567...` failure-path replay + `xcrun metal -c`、`FORCE_PLAYTOOLS_REBUILD=1 ./BuildScripts/sync_playtools_xcframework.sh`、`./BuildScripts/build_and_install.sh` 以及 fresh `case E` 复测。最新一轮 `processLaunchId=launch-15751-41e75001-ec5e-4b60-be84-659e600fe989` 中，`f567...` 已不再出现在 latest failure surface；新的 latest/actionable compile blocker 已前移到 `4010578BBE3B30E1_4673 / e3c0894b45454fb3dbdb7d07523ed6fd2645c3e6b8939b11c02d3709f70df905`，compiler message 主体为 `mtl_FrontFace` 未声明。**当前默认 gate 已更新为：优先消费 `4010578... / e3c089...` failure-path 样本做离线 replay / 最小样本化 / `air.front_facing` builtin lowering 修复，再回到 fresh `case E` 复测。** |
...
> **优先级更新（2026-04-07）**：当前主线已切换为 **`E-006g` → `E-006f` → `E-006e`**。`E-006d` 因确认为偶现问题，暂时搁置并保留进度；`E-006a` / `E-007` 继续下调一级。补充说明：`E-006f2` 虽编号早于 `E-006f3`，但它是需要用户确认的工作区外专项分支；默认执行顺序仍为 **`E-006f1 -> E-006f3 -> E-006f2 -> E-006f4`**。
...
| E-006g3 | ↳ 把 `compile_failed` 命中面收缩到最小 `cacheKey` / selector / module 集合，为 `E-006g4` 准备最小修复 / 旁路面 | IN PROGRESS（fresh `case E` 已完成，`D4CA... / f567...` 已退出 latest surface；当前默认入口前移到 **`4010578... / e3c089...` 的 `mtl_FrontFace` / `air.front_facing` builtin lowering 问题**） | [E-006g](E-006g-LoveAndDeepspaceTripleToggleStartupCrash.md) |
...
- **`恋与深空` 当前默认入口仍是 `E-006g3`，但历史 blocker 前移细节已下沉**：不要再把“补矩阵脚本 / 补埋点”当作默认任务；当前只需记住 `4010578... / e3c089...` 是最新 actionable compile blocker，默认动作是先用 failure-path 样本做离线 replay / 最小样本化，再回到 fresh `case E`。更早的 `791A...`、`F474...`、`A101...`、`45AE...`、`8ABA...` 与 `D4CA...` 前移脉络统一下沉到 `E-006g-Archive.md`
