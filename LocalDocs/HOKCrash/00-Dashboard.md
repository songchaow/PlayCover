...
- **当前状态**：`com.tencent.ngr` 在"仅改动 PlayCover/PlayTools、不动
  app 二进制"的约束下已经**稳定启动到主游戏窗口**，60s 长跑无新
  `NGR-*.ips`。兜底链路：
  1. `HOK-013`：PlayTools `__attribute__((constructor))` 最早时刻为
     `0x10e2146f8` 写入一个 stub object 地址，让 NGR dyld initializer
     里的 `ldr x8, [x19]; ldr x8, [x8, #0x10]; blr x8` reader 链安全
     no-op。（`LocalDocs/HOKCrash/HOK-013-slot-preheat.md`）
  2. `HOK-010`：`PlaySettings.rootWorkDir` 从
     `disableForMinimalStartupCompat(...)` 摘除；`PlayApp.launch()`
     在 `settings.sync()` 之后 self-healing 把 NGR 的 `rootWorkDir`
     强制为 `true`，让 UE4 的相对路径以 `/` 为基准。（`LocalDocs/HOKCrash/HOK-014-alert-suppressor.md`）
  3. `HOK-014`：PlayTools 层 swizzle `-[UIViewController
     presentViewController:animated:completion:]`，对
     `UIAlertController` 实例直接调 `completion(nil)` 返回、不走
     AppKit sheet 路径。拦下的 alert 是 `title="Message"` /
     `message="QtsFileSystem Create Failed!!"`（UE4 command-line 早
     读触发的 3 连 fatal 的 UI 兜底）。（`LocalDocs/HOKCrash/HOK-014-alert-suppressor.md`）

  HOK-007B 候选 E（NGR app 二进制 4 字节 patch）已 **revert**（
  `state=original`）且不再纳入日常兼容链；HOK-013 + HOK-014 联合足以
  兜住启动路径。

- **当前已知事实**（只列现在对维护仍有意义的）：
  - faulting PC `0x10480df08` / `x19=0` 的原空指针 deref，在 HOK-013
    stub 生效下已不触发。
  - `0x10e2146f8` 的 writer 是函数 `0x103a29c60`（不是 HOK-011 最初
    标的 `0x103a29b7c`，那是相邻的 Logger dispatch wrapper），写入
    点 `0x103a29f7c`；NGR 自身 `__init_offsets` 反向 BFS 不可达，iOS
    下由外部 framework/ObjC/跨 dylib 链触发。HOK-013 之后**不再依赖
    识别实际 writer** 就能让 app 跑起来。
  - ObjC 重复类警告：`PxFrameworkLoader` / `AReachability` /
    `PxDyLibFrameworkLoader` 在两个 image 同名；macOS vs iOS 挑选规则
    不同是 iOS 行为差异的强候选，**但当前不需要为此做任何修复**（
    HOK-013 绕过 + HOK-014 压制已经让差异对用户不可见）。
  ...
  - PlayCover GUI 端 `AppSettings.settings` 有 `didSet → encode()`，
    启动 app 时 `settings.sync()` 会触发写回 plist；HOK-010 follow-up
    已在 `PlayApp.launch()` 里对 NGR self-heal
    `settings.settings.rootWorkDir=true`，避免 stale 内存把 plist
    改回 false。
  - 候选 E patch runner：`Scripts/hok007b_ngr_patch_runner.py`，当前
    `state=original`；`build/hok-007b-backups/*.bin` 保留作为历史
    备份，日常不 apply。

- **当前主线**：无硬主线。app 启动链路已稳定，保持观察态。

- **当前卡点**：无。

- **下一步默认规划**：
  1. 无强制下一步；保持观察态。若用户发起"尝试登录 / 进游戏"验证
     （触发 HOK-009 条件），必须先明确告知用户这需要手工 UI 操作。
  2. （可选）`HOK-008`：把"确保候选 E revert → plist `rootWorkDir=1`
     → 启动 NGR → 跑 baseline + `launch-events.jsonl` + 检查
     `NGR-*.ips` + 区分 blockingDialogs 是主窗口还是 sheet"这条链路
     固化成单个脚本，取代目前需要人工组合 `hok004` / `hok006` /
     `hok007b` 的现状。
  3. （可选）若主游戏窗口后续出现新崩溃或新类型 alert，按 HOK-014
     swizzle 套路扩展压制白名单；按 HOK-013 套路扩展新的 slot 预热
     清单。
...
| HOK-013 | DONE | PlayTools constructor 最早时刻为 NGR 写入 stub object 地址到 `0x10e2146f8`；bundle-scoped + slide 安全阀 + `dispatch_once` 幂等；诊断事件 `hok013_ngr_slot_preheat` | `HOK-013-slot-preheat.md` |
| HOK-014 | DONE | PlayTools 层 swizzle `-[UIViewController presentViewController:animated:completion:]`，对 `UIAlertController` 直接 `completion(nil)` 返回。含 HOK-010 落地（`rootWorkDir` 摘出 compat 强制关闭 + `PlayApp.launch()` self-heal） | `HOK-014-alert-suppressor.md` |
...
- **`launch_app_with_lldb` headless 结构化证据**：消费 `lldb.stopReason`
  ...
  fault 字段 = 证据有效。
- **`blockingDialogs >= 1` 不等于回归**：b.0 gate 对"NGR onscreen
  window"本身也会报 1。结合 window size 判断：`~260×204` 是 sheet
  （问题），`1478×859` 是主游戏窗口（正常）。
