...
| 当前 blocker 与下一步（2026-04-05） | 真正的 blocker 已从“为什么没有 `replacement_attempt`”进一步收敛为两件事：1）fresh 原神 on-run 在本轮 `BridgeListener` **自动重试 / 断线重注册** 修复后，仍表现为 **app 进程存活、`list_sessions` 为空、MCP `create_session(timeout=120)` 请求超时**，且最近 unified log 中仍未看到 `PlayTools` / `BridgeListener` 关键字；这说明当前要优先解释的，已经不只是“注册失败是否需要重试”，而是 **fresh 注入后的 runtime 是否根本没有稳定进入 `PlayCover.launch()` / `BridgeListener.start()`**；2）`replacement-on-run3/on4` 中其余 `55` 次失败 attempt 仍落为 `reasonCode=exception`，`detail="Failed to launch llvm-dis: Operation not permitted"`，说明当前仍是**部分替换成功 + 部分 `llvm-dis` 权限失败**。下一步应优先用现有自动化路径继续收敛这两条线，而不是新增需要人工介入的日常 gate；只有先恢复**至少一轮 `replacement=on` 且带 `.gputrace + aggregate source` 的稳定样本**，再继续回答“替换 vs 不替换”是否稳定不同 |
...
|  | 当前已拿到 `off1/off2/on1/on2/on3/on4` 六轮快照：已稳定排除 **captured corpus / 单模块 artifact 层面的 drift**，且标准 build/install 后的 `replacement-on-run3/on4` 都保持 `replacement_attempt=91` / `replacement=36`；其中本轮新增的 `LLVMDisassembler` host bridge **重试 + 防误 fallback** 未引入新的 artifact 回归，`on4` 快照摘要与 `on3` 一致。但**跨模式稳定不同仍未成立**，fresh on-run 依旧没有新的 `.gputrace`。当前下一步应先把 agent 可独立完成的 fresh `replacement=on` live 链路重新稳定下来：优先下钻 **session 为什么会先表现为 `ready` 后于 `get_capture_status` / `capture_metal_frame` 前后超时并丢失，而本轮又前移成 `create_session(timeout=120)` 未等到 runtime 注册**，并并行收敛 `55` 个 `reasonCode=exception` / `detail="Failed to launch llvm-dis: Operation not permitted"` 的失败 attempt；在恢复至少一轮 `replacement=on` 且带 `.gputrace + aggregate source` 的稳定样本前，不应继续把结论过早推向 shader lowering 或更后续 pipeline，也不应把人工协助升级为日常前提。 | | |
