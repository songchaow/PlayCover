...
- **E-006**：`LLVMDisassembler` 的 runtime→host `llvm-dis` bridge、`makeLibrary(source:)` 前的 MSL preflight guard，以及 `E-006a2c1` 的 `Toucher.touchcam` 空值防护均已落地，并已通过 `FORCE_PLAYTOOLS_REBUILD=1 ./BuildScripts/sync_playtools_xcframework.sh`、`./BuildScripts/build_gui.sh`、`./BuildScripts/test_mcp.sh RegistrationListenerHostCommandTests` 验证构建与桥接链路。2026-04-03 基于这版构建完成了 `E-006a2c2` 的 **关闭 `keymapping` 后 2 轮受控 live 复测**，确认 toucher / keymapping 干扰已基本从 shader 主线中剥离；与此同时，`ShaderSourceDiagnostics` 暴露的 preflight / compile blocker 继续收敛到 `IRToMSLConverter`。在此前 `E-006a2d1`、`E-006a2d2a` 的 packed-return / 参数映射 / fast-math 修复基础上，本轮继续完成 **`E-006a2d2b`**：1) `splitMetadataTokens` 现会正确处理带逗号的 metadata 字符串（如 `texture2d<float, sample>`）；2) `buildParametersFromMetadata` / `setupParameterMappings` 现保留并按 metadata `argIndex` 精确回接 `%0..%N`；3) fragment 的 `air.position` + `air.fragment_input` 现会合成为专用 `stage_in` struct；4) `generateMSL` 会补发 `Uniforms` / `Particle` 等自定义 struct 定义；5) `parseStructFieldInfoFromMetadata` 已修正为从真实 node content 提取 `air.struct_type_info`。已用离线 harness 对 `test_extractvalue.ll` 复测：生成结果已恢复 `texture2d<float> tex [[texture(0)]]`、`sampler smp [[sampler(0)]]`、`stageIn.texcoord`、`uniforms.modelViewProjection[...]`、`particles[tid].position/velocity`；其中 fragment-only 片段已通过 `xcrun --sdk macosx metal -c`，同时本轮仍通过 `FORCE_PLAYTOOLS_REBUILD=1 ./BuildScripts/sync_playtools_xcframework.sh`。这说明当前 blocker 已从 texture/sampler 与自定义 struct 定义缺失，进一步前移到 **vertex aggregate return** 与 **float4→float3 向量维度收敛**。
...
|  | runtime→host 的 `llvm-dis` bridge、聚合 MSL preflight guard，以及 keymapping 隔离 live 复测均已完成；本轮继续完成 `E-006a2d2b`，离线把 texture/sampler 形参与自定义 struct 字段命名/定义打通。下一步新拆 `E-006a2d3`，继续修 vertex aggregate return 与 float4→float3 向量维度问题，再决定是否进入下一轮 live 截帧复测 | | |
...
|  | 已完成 `E-006a2d1`、`E-006a2d2a`、`E-006a2d2b`，离线把 `%...` / `<N x T>` / `fadd/fmul/fdiv fast` / 空 RHS / 坏 texture-sampler 形参 / `.fieldN` 缺定义 这几类 blocker 清掉；`test_extractvalue.ll` 生成结果现已恢复 `texture2d<float>`、`sampler`、fragment `stage_in`、`Uniforms.modelViewProjection`、`Particle.position/velocity`。下一步拆到 `E-006a2d3`，继续修 vertex aggregate return 与 float4→float3 向量维度问题 | | |
...
| E-006a2d3 | ↳ vertex aggregate return + 向量维度收敛 | TODO | |
|  | 当前 `test_extractvalue.ll` 的离线全量编译剩余错误已收敛到 `return { t16, t17 };` 的 vertex aggregate return，以及 `float4(...) * float3` 这类向量维度不匹配；先修这两类，再评估是否进入下一轮 live 复测 | | |
