...
- **E-006d（当前最高优先级）**：调查“用 PlayCover 打开原神，在同一界面重复启动时，画面表现每次都不完全一样；mesh 不变，但局部渲染结果异常”的现象。当前**不能实锤是 shader 改坏**：因为原神是延迟管线，base pass 对比看起来也可能类似，异常也可能来自后处理、着色阶段，或更上层的 render pipeline 顺序 / 配置差异。最新 `off1/off2/on1/on2/on3` 五轮快照继续稳定排除了 **captured corpus / 单模块 artifact 层面的随机漂移**，但**仍未形成“跨模式稳定不同”的 corpus / trace 级证据**；因此 `replacement=off / on` 仍是最低风险的对照口径，但不能再表述成“已经稳定证明差异存在”的结论。
- **当前最该做的事**：`E-006d8` 的第一轮 run matrix 已经够用，下一步**不是**继续机械补 run，也不是回头扩 lowering；而是把 fresh `replacement=on` 的自动化 live 链路重新稳定下来。当前最窄、也最该优先处理的两条 blocker 是：1）`create_session -> get_capture_status / capture_metal_frame` 仍会在 `ready` 后超时或直接丢 session，拦住新的 `.gputrace`；2）`replacement-on-run3` 已恢复 `replacement_attempt=91` / `replacement=36`，但其余 `55` 次失败都落为 `reasonCode=exception`，`detail="Failed to launch llvm-dis: Operation not permitted"`。在这两条线收敛前，不应继续扩 run、也不应把日常 gate 推回人工协助；只有先在 agent 可独立完成的流程内恢复**至少一轮 `replacement=on` 且带 `.gputrace + aggregate source` 的稳定样本**，矩阵才有资格继续回答“替换 vs 不替换”是否稳定不同。
...
| 当前 `E-006d8` 第一层矩阵结论（2026-04-05） | `off1/off2/on1/on2/on3` 五轮快照已经足够证明：**同模式输入稳定**、共享 `moduleKey` 为 **91/91**、单模块 `.bc/.ll/.metal` 本体未出现语义级随机漂移；`build/e006d-run-matrix.json` 最新摘要为：`groups.off.pairSummary.allPairsInputStable=true`、`groups.on.pairSummary.allPairsInputStable=true`、`crossMode.offVsOnPairSummary.allPairsDifferent=false`、`differentPairCount=4` |
| 当前尚未证明的部分（2026-04-05） | 现阶段**仍未形成“跨模式稳定不同”的 corpus / trace 级证据**：fresh `replacement-on-run3` 虽已恢复 aggregate source，但仍未拿到新的 `.gputrace`；因此当前还不能把结论提前收敛为“替换开启后一定稳定引入了最终差异” |
| 当前 replacement 证据基线（2026-04-05） | 标准 `BuildScripts/build_and_install.sh` + fresh `replacement-on-run3` 已恢复 runtime replacement 命中证据：快照摘要为 `manifestLines=1636`、`modules=91`、`replacements=36`、`diagnostics=18`；manifest 统计为 `replacement_attempt=91`（`succeeded=36`、`failed=55`），说明当前已不再是“enabled 但没有 attempt/aggregate” |
| 当前 blocker 与下一步（2026-04-05） | 真正的 blocker 已从“为什么没有 `replacement_attempt`”进一步收敛为两件事：1）`create_session -> get_capture_status / capture_metal_frame` 仍会在 `session=ready` 后超时或直接丢 session，导致 fresh on-run 依旧没有新的 `.gputrace`；2）`replacement-on-run3` 中其余 `55` 次失败 attempt 全部落为 `reasonCode=exception`，`detail="Failed to launch llvm-dis: Operation not permitted"`，说明当前是**部分替换成功 + 部分 `llvm-dis` 权限失败**。下一步应优先用现有自动化路径继续收敛这两条线，而不是新增需要人工介入的日常 gate；只有先恢复**至少一轮 `replacement=on` 且带 `.gputrace + aggregate source` 的稳定样本**，再继续回答“替换 vs 不替换”是否稳定不同 |
...
|  | 当前已拿到 `off1/off2/on1/on2/on3` 五轮快照：已稳定排除 **captured corpus / 单模块 artifact 层面的 drift**，且标准 build/install 后的 `replacement-on-run3` 已恢复 `replacement_attempt` 与 aggregate source（`replacement_attempt=91`、`replacement=36`）；但**跨模式稳定不同仍未成立**，fresh on-run 依旧没有新的 `.gputrace`。当前下一步应先把 agent 可独立完成的 fresh `replacement=on` live 链路重新稳定下来：优先下钻 **session 为什么会在 `ready` 后于 `get_capture_status` / `capture_metal_frame` 前后超时并丢失**，并并行收敛 `55` 个 `reasonCode=exception` / `detail="Failed to launch llvm-dis: Operation not permitted"` 的失败 attempt；在恢复至少一轮 `replacement=on` 且带 `.gputrace + aggregate source` 的稳定样本前，不应继续把结论过早推向 shader lowering 或更后续 pipeline，也不应把人工协助升级为日常前提。 | | |
...
- **`session ready` 不是“capture-ready 且稳定”的充分条件**：`replacement-on-run3` 已再次复现 `create_session=ready → get_capture_status timeout → capture_metal_frame session not found → list_sessions 为空`，因此 live 收尾仍要同时核对 session 状态、capture 目录、`DiagnosticReports` 与对应快照是否完整固化
