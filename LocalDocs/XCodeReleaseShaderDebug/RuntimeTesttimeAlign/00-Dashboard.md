...
| `RTA-004` 收口 `original IR -> compile request` 的单一实现 | DONE | runtime、`mtl-device` harness 与 Python `xcrun` backend 不再各写一份 posture / arg inference；compile 规则修改时默认只需改一处 shared planner | shared planner 实现与 cross-backend regression matrix 已收口，后续 compile 规则变更默认只改一处 |
...
| `RTA-004.5` 补 shared planner 回归与跨 backend 一致性测试 | DONE | single-module / multi-module / user override / `xcrun` / `mtl-device` 的 plan 输出与 compile summary 字段都能稳定对齐 | planner 单测、`mtl-device` harness 实测，以及 aggregate `xcrun` / `mtl-device` CLI cross-backend matrix 已覆盖当前 fast-math reason-code 契约 |
...
| `RTA-004.5.2` 补 aggregate `xcrun` 入口与 CLI 级 cross-backend compile summary 断言矩阵 | DONE | aggregate `xcrun` compile summary 与 `mtl-device` 在 conflict / partial / unavailable / override 上都有更直接的端到端断言 | 已补 shared case matrix、aggregate `xcrun` summary mock matrix，以及 `xcrun` / `mtl-device` CLI reason-code cross-backend assertions |
| `RTA-003` 在链路对齐后恢复上游高风险 case 推进 | TODO | 在 `RTA-004` 完成后，再继续推进 `SemanticsValidation` 中剩余 `entry 参数` family 的 case-by-case 收敛 | `RTA-004` 已收尾，下一轮切回上游 `CC-003.9` |
