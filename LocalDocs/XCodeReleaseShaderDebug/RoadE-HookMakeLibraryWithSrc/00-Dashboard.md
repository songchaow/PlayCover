...
- **`E-006g`（当前最高优先级）**：解决 `恋与深空` 在同时启用 `metal capture + startup injection + shader replacement` 时的启动崩溃。**`791A306ED1B6648B_4577 / ec0c6f...` 与 `F474... / bbb32d...`、`A101... / 82d1...` 已先后退出 latest fresh blocker；本轮又把 `45AE24662B56C487_14497 / 1cdc9318...` 的三类 lowering 缺口（`bool3 select`、global-const-array `GEP`、`air.gather_texture_2d`）补齐，并完成真实 failure module replay + `xcrun metal -c`、新增最小回归样本 replay + `xcrun metal -c`、以及 `FORCE_PLAYTOOLS_REBUILD=1 ./BuildScripts/sync_playtools_xcframework.sh` 验证。当前状态更新为“代码已落、离线 replay / build 已通过，待 fresh case E 验证 latest failure surface 是否继续前移”。** 详细路径见 `E-006g-LoveAndDeepspaceTripleToggleStartupCrash.md`。
...
| `恋与深空` 三开关启动兼容性 blocker（2026-04-07） | `B/C` 可在 launch 后继续存活，`D/E` 会在 `playcover_launch_complete` 之后约 7~8 秒内 late crash。`791A... / ec0c6f...`、`F474... / bbb32d...`、`A101... / 82d1...` 已先后退出 latest compile blocker；本轮又把 `45AE24662B56C487_14497 / 1cdc9318...` 的 `bool3 select`、global-const-array `GEP` 与 `air.gather_texture_2d` placeholder 通过离线 replay/compile 修复为 green，并新增最小回归样本。**当前默认 gate 已更新为：做 fresh `case E` 复测，确认 `45AE... / 1cdc...` 是否退出 latest failure surface，并观察新的 actionable blocker 是否继续前移。** |
...
| E-006g3 | ↳ 把 `compile_failed` 命中面收缩到最小 `cacheKey` / selector / module 集合，为 `E-006g4` 准备最小修复 / 旁路面 | IN PROGRESS（`45AE... / 1cdc...` 已完成代码修复 + replay / compile + `sync_playtools_xcframework` 验证；当前唯一默认入口改为 **fresh `case E` 复测并确认 latest failure surface 是否继续前移**） | [E-006g](E-006g-LoveAndDeepspaceTripleToggleStartupCrash.md) |
...
- **`恋与深空` 当前默认入口仍是 `E-006g3`，但 latest blocker 已继续前移**：不要再把“补矩阵脚本 / 补埋点”当作默认任务；`791A... / ec0c6f...`、`F474... / bbb32d...`、`A101... / 82d1...` 与 `45AE... / 1cdc...` 现都应转为**离线回归样本**。其中 `45AE... / 1cdc...` 的 `bool3 select`、global-const-array `GEP` 与 `air.gather_texture_2d` placeholder 已在本轮离线修复并编绿；当前真正需要回答的，是它在 fresh `case E` 中是否已退出 latest failure surface，以及新的 actionable blocker 会前移到哪里
...
- **更细的历史基线、lowering 经验与已完成轮次已统一下沉到独立参考文档**：见 `00-Dashboard-Archive.md`、`E-004-MetallibSourceExtraction-Archive.md`、`E-006g-LoveAndDeepspaceTripleToggleStartupCrash.md`、`E-006e-QQSpeedCaptureReplacementStartupCrash.md` 与 `E-006f-GenshinIntegrityCheck-314302.md`
