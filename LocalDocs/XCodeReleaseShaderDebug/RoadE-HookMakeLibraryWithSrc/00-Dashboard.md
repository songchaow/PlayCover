...
- **`E-006g`（当前最高优先级）**：解决 `恋与深空` 在同时启用 `metal capture + startup injection + shader replacement` 时的启动崩溃。**当前默认入口已经固定**：先消费 `4010578BBE3B30E1_4673 / e3c0894b45454fb3dbdb7d07523ed6fd2645c3e6b8939b11c02d3709f70df905` failure-path `module.ll` 做离线 replay / `xcrun metal -c`，把 `mtl_FrontFace` / `air.front_facing` 邻域缩成最小 `test-data` 样本，再回到 fresh `case E` 复测。已退出的历史 blocker 与前移脉络统一下沉到 `E-006g-Archive.md`。详细路径见 `E-006g-LoveAndDeepspaceTripleToggleStartupCrash.md`。
...
| `恋与深空` 三开关启动兼容性 blocker（2026-04-08） | `B/C` 可在 launch 后继续存活，`D/E` 会在 `playcover_launch_complete` 之后约 7~8 秒内 late crash。历史 compile blocker 前移脉络已统一下沉到 `E-006g-Archive.md`；当前 latest/actionable compile blocker 固定为 `4010578BBE3B30E1_4673 / e3c0894b45454fb3dbdb7d07523ed6fd2645c3e6b8939b11c02d3709f70df905`，compiler message 主体为 `mtl_FrontFace` 未声明。**当前默认 gate**：优先对该 failure-path 样本做离线 replay / 最小样本化 / `air.front_facing` builtin lowering 修复，再回到 fresh `case E` 复测。 |
...
> **优先级更新（2026-04-08）**：当前主线已切换为 **`E-006g` → `E-006f` → `E-006e`**。`E-006d` 因确认为偶现问题，暂时搁置并保留进度；`E-006a` / `E-007` 继续下调一级。当前 `E-006g` 的默认入口固定为 **围绕 `4010578... / e3c089...` 做 compiler-first 最小样本化与 `air.front_facing` builtin lowering**。补充说明：`E-006f2` 虽编号早于 `E-006f3`，但它是需要用户确认的工作区外专项分支；默认执行顺序仍为 **`E-006f1 -> E-006f3 -> E-006f2 -> E-006f4`**。
...
| E-006g3 | ↳ 把 `compile_failed` 命中面收缩到最小 `cacheKey` / selector / module 集合，为 `E-006g4` 准备最小修复 / 旁路面 | IN PROGRESS（当前默认入口：**`4010578... / e3c089...` failure-path 样本的 compiler-first 最小样本化与 `air.front_facing` builtin lowering**；更早 blocker 前移脉络统一下沉到 `E-006g-Archive.md`） | [E-006g](E-006g-LoveAndDeepspaceTripleToggleStartupCrash.md) |
...
- **`恋与深空` 当前默认入口仍是 `E-006g3`，但历史 blocker 前移细节已下沉**：不要再把“补矩阵脚本 / 补埋点”当作默认任务；当前只需记住 `4010578... / e3c089...` 是最新 actionable compile blocker，默认动作是先用 failure-path 样本做离线 replay / 最小样本化，再回到 fresh `case E`。更早的 `791A...`、`F474...`、`A101...`、`45AE...`、`8ABA...` 与 `D4CA...` **均已退出 latest surface**，其前移脉络统一下沉到 `E-006g-Archive.md`
