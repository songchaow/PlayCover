...
- **当前结论**：`HOK-004`、`HOK-005A`–`HOK-005D`、`HOK-006`、`HOK-007A`、`HOK-007B`、`HOK-011`、`HOK-012-A`、`HOK-012-B`、`HOK-012-C.1`（legacy baseline）、`HOK-012-C.2`（pre-run watchpoint live run）、`HOK-012-C.3-a`（deferred-install watchpoint 自动化）均已完成。当前对 `com.tencent.ngr` 的理解是：候选 E patch 仍 apply，原 `0x10480df08` faulting window 已被压成 no-op；app 能跑过 PlayTools compat 序列、`GCloudCore` / `PluginManager` / `CAppLifecycleLaunch` C++ ctor 与 `SceneExtension hook_frame` 循环，随后触发 UE4 fatal（先 `Line:34`，再 `QtsFileSystem Create failed.` / `ICU data directory was not discovered`），`abort()` 弹出 Apple 崩溃对话框。HOK-011 的 "`0x10e2146f8` 唯一 store = `0x103a29f7c`，唯一 reach 入口 = `0x103a29b7c`" 静态结论**仍然成立**；HOK-012-C.2/C.3-a 两轮 live 都没有直接抓到 writer 被调用的证据，但也没有有效证据反驳这条链——两轮 LLDB 的捕获窗口都小于 launch → `abort()` 所需时间，且 LLDB 默认对 SIGABRT 是 pass 的，于是 `abort()` 真实发生时 LLDB 早已处于"在等用户点 Apple 对话框"的被动等待状态。HOK-012-A/B/C 的自动化栈已就绪并保持 legacy 行为 100% 兼容（host 侧 `PlayApp` 与 MCP `LaunchService` 两处 `effectiveLaunchEnvironment` 的 bundle-scoped `DYLD_PRINT_INITIALIZERS=1` / `DYLD_PRINT_APIS=0` 注入、headless LLDB runner + `launch_app_with_lldb` MCP 工具 + `hok006_ngr_lldb_runner.py` 脚本四层同步扩展的 `watchAddress` / `watchSize` / `preRunCommands` / `dyldInitializersLogPath` / `deferWatchpointInstall` / `--writer-address`、`process launch -e` 子进程 stderr 重定向、watchpoint 模式 stop→continue 循环、默认地址 `0x10e2146f8` / size 8 / log `build/hok-012-ngr-dyld-initializers.log` / report `build/hok-012-ngr-watchpoint-report.json`、`LaunchServiceTests` 26 个用例全过、`BuildScripts/build_and_install.sh` 构建+安装+ad-hoc 签名成功）。主线停在 `HOK-012-C.3-b`：让 LLDB 主动拦 SIGABRT，在对话框出现**之前**把 abort 停在 LLDB 层，然后一次性读 `0x10e2146f8` 当前值、writer bp 的 hit count、watchpoint 命中记录。
- **当前已知事实**：
...
- **当前主线**：`HOK-012-C.3-b`——让 LLDB 主动拦住 UE4 `abort()`，在 Apple 崩溃对话框出现**之前**把 abort 停在 LLDB 层，然后在 abort stop 上一次性采集完整证据：所有线程 backtrace + `memory read 0x10e2146f8` + `breakpoint list`（看 `0x103a29b7c` / `0x107e5df10` 的 hit count）+ `watchpoint list`（看 writer 曾否触发 watchpoint）。核心动作是 `process handle -s true -n true -p false SIGABRT`；`--lldb-timeout` 只需保守值（60s 经验够用）容纳 launch → abort 段，不与 `.ips` 时间挂钩（`.ips` 里的退出时刻 = 用户点对话框时刻，不是 app 的自然崩溃时刻）。本步不引入新 MCP schema，只扩 Python runner 的 defer-mode 默认脚本。
- **当前卡点**：
