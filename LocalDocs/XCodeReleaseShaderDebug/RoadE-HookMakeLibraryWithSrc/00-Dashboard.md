...
- **当前最该做的事**：`E-006d8` 的第一轮 run matrix 已经够用，下一步**不是**继续机械补 run，也不是回头扩 lowering；而是优先收敛 fresh `replacement-on-run3` 暴露出来的两个更窄 blocker：1）runtime replacement 证据已经恢复，但当前只做到了**部分成功替换**——`replacement_attempt=91`、`replacement=36`，其余 `55` 次失败都落为 `reasonCode=exception`，`detail="Failed to launch llvm-dis: Operation not permitted"`；2）`create_session -> get_capture_status / capture_metal_frame` 仍会在 `ready` 后超时或直接丢 session，导致 fresh on-run 依旧拿不到新的 `.gputrace`。只有先恢复**至少一轮 `replacement=on` 且带 `.gputrace + aggregate source` 的稳定样本**，矩阵才有资格继续回答“替换 vs 不替换”是否稳定不同。
...
|  | 当前已拿到 `off1/off2/on1/on2/on3` 五轮快照：已稳定排除 **captured corpus / 单模块 artifact 层面的 drift**，且标准 build/install 后的 `replacement-on-run3` 已恢复 `replacement_attempt` 与 aggregate source（`replacement_attempt=91`、`replacement=36`）；但**跨模式稳定不同仍未成立**，fresh on-run 依旧没有新的 `.gputrace`。当前下一步应优先下钻 **session 为什么会在 `ready` 后于 `get_capture_status` / `capture_metal_frame` 前后超时并丢失**，并并行收敛 `55` 个 `reasonCode=exception` / `detail="Failed to launch llvm-dis: Operation not permitted"` 的失败 attempt；在恢复至少一轮 `replacement=on` 且带 `.gputrace + aggregate source` 的稳定样本前，不应继续把结论过早推向 shader lowering 或更后续 pipeline。 | | |
