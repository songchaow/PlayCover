...
- **E-006d（当前最高优先级）**：调查“用 PlayCover 打开原神，在同一界面重复启动时，画面表现每次都不完全一样；mesh 不变，但局部渲染结果异常”的现象。当前**不能实锤是 shader 改坏**：因为原神是延迟管线，base pass 对比看起来也可能类似，异常也可能来自后处理、着色阶段，或更上层的 render pipeline 顺序 / 配置差异。最新 `off1/off2/on1/on2/on3/on4/on5` 七轮快照继续稳定排除了 **captured corpus / 单模块 artifact 层面的随机漂移**，但**仍未形成“跨模式稳定不同”的 corpus / trace 级证据**；因此 `replacement=off / on` 仍是最低风险的对照口径，但不能再表述成“已经稳定证明差异存在”的结论。
- **当前最该做的事**：`E-006d8` 的第一轮 run matrix 已经够用，下一步不再是继续机械补 run，也不是回头扩 lowering。2026-04-06 本轮已按标准路径完成 fresh `replacement-on-run6`：`FORCE_PLAYTOOLS_REBUILD=1 ./BuildScripts/sync_playtools_xcframework.sh` → `./BuildScripts/build_and_install.sh` → `Scripts/set_shader_replacement_mode.py --bundle-id com.miHoYo.Yuanshen --mode on` → `Scripts/e006d_matrix_runner.py prepare-run --bundle-id com.miHoYo.Yuanshen --mode on --run-index 6` → `remove_playtools / inject_playtools / launch_app / create_session(timeout=30)`；结果再次成功拿到 `session=ready`，`RuntimeLaunchDiagnostics/<bundleId>/launch-events.jsonl` 也继续明确命中了 `playcover_launch_enter`、`bridge_listener_command_listener_ready`、`bridge_registration_established` 与 `playcover_launch_complete`，说明 fresh 注入后的 runtime **确实已经进入 `PlayCover.launch()` / `BridgeListener.start()` 并建立 registration channel**。本轮同时对 `LLVMDisassembler` 做了 host bridge 重试 / fallback 调整：`sessionUnavailable` 不再回退到本地 `posix_spawn`，因此 `replacement-on-run6` 最新增量里的 failure 已**不再前移为** `Failed to launch llvm-dis: Operation not permitted`，而是前移为 `Host bridge reported an error: Session not registered: <sessionId>`；这说明“session 未就绪 → 本地 spawn → sandbox EPERM”这条错误分支已被压掉，但 host bridge **session 生命周期 / command reachability** 仍未稳定。2026-04-06 随后又补了一轮 host 侧修复：`SessionHealthMonitor` 移除 stale session 时，会同步通过 `RegistrationListener.disconnectSessions(...)` 主动断开对应 runtime 的 registration channel，而不再只在 registry 里静默删除；这样 runtime 一旦命中 host 侧 stale cleanup，会立即感知连接被关闭并触发标准 re-register，而不是继续拿着已被 host 删除的 `sessionId` 发送 host bridge 命令。该修复当前已由 `PlayCoverMCPTests` 的集成用例覆盖，但**尚未经过新的 live on-run 复测**。当前真正的 blocker 因此收敛为：1）带着这条 host-side split-brain 修复重新做 fresh on-run，确认 `Session not registered` 是否收敛；2）`capture_metal_frame` 在**自定义工作区输出路径**上仍会遇到权限失败，只能先走目标 app 容器默认 `Captures/` 再纳入快照；3）trace 侧合法 MSL 覆盖仍偏低（上一轮成功 `.gputrace` 仍只有 `11` 个源码文件中的 `2` 个合法 MSL）。在这三条线收敛前，不应回头把主 blocker 重新定性为 launch/bootstrap 问题，也不应把日常 gate 推回人工协助。
...
| 当前 blocker 与下一步（2026-04-06） | 真正的 blocker 已从“fresh 注入后 runtime 可能根本没进 `PlayCover.launch()`”继续前移为三件事：1）host 侧已补上 stale-session cleanup 时主动断开 registration channel 的修复，下一步要带着该修复重新做 fresh on-run，确认 `Session not registered` 是否从“host/runtime split-brain”进一步收敛为更窄的 reachability 问题；2）`capture_metal_frame` 使用自定义工作区 `output_path` 会因权限失败，只能先导出到目标 app 容器默认 `Captures/` 后再纳入标准快照；3）上一轮成功 `.gputrace` 仍只有 `2/11` 个合法 MSL，说明 live/capture 链路恢复后，最终 trace 侧源码覆盖仍偏低。下一步应优先沿这三条自动化路径继续收敛，而不是再把重心放回“入口有没有跑到”或重新引入人工 gate |
