...
- **当前结论**：`HOK-004`、`HOK-005A`–`HOK-005D`、`HOK-006`、`HOK-007A`、`HOK-007B`、`HOK-011`、`HOK-012-A`、`HOK-012-B`、`HOK-012-C.1`（legacy baseline）、`HOK-012-C.2`（pre-run watchpoint）、`HOK-012-C.3-a`（deferred-install watchpoint）、`HOK-012-C.3-b.0`（对话框检测 + 残留进程强杀 + 污染 gate）、`HOK-012-C.3-b.1`（SIGABRT 拦截 + abort-stop handler 四层自动化）、`HOK-012-C.3-b.2`（`runLLDBHeadless` 收尾窗口参数化为 `teardownTimeoutSeconds`）、`HOK-012-C.3-b.3`（`0x10e2146f8` 未污染 live 证据采集）均已完成。**`com.tencent.ngr` 启动期阻塞窗口的结构**：`NSAlert` 以 sheet modal 形式 attach 到 parent window，order 路径是 `-[NSAlert beginSheetModalForWindow:completionHandler:]_block_invoke` → `-[NSWindow _beginWindowBlockingModalSessionForSheet:service:completionHandler:isCritical:]` → `-[NSApplication _orderFrontModalWindow:relativeToWindow:]` → `-[NSWindow(NSSheets) _orderFrontRelativeToWindow:]` → `-[NSWindow orderFront:]`（`windowLayer=0` 是 sheet 特征）；sheet 一旦 order front，UI 线程进入 sheet modal session，其他线程继续跑但永不前进，所有 LLDB 读数变为冻结态快照。关键拦截 bp 是 `-[NSWindow orderFront:]`，`-[NSApplication runModalForWindow:]` 与 `CGSOrderWindow`（→ `SLSOrderWindow`）作为冷备覆盖非 sheet 的 modal 路径。**`0x10e2146f8` 的 live 归因**（`build/hok-012-c-3-b-3-report.json`，`blockingDialogs=0 residualPIDsKilled=0 lldbBlockingDialogDetected=False lldbAutomationReady=True`）：SIGABRT 拦截让 abort-handler 在 app 进入 UE4 fatal abort 时接管现场（此时 `BqCCS` / `[GPM] cpp constructor` 等 static init 已跑但 sheet 尚未 order front），transcript 明确给出 `0x10e2146f8: 0x0000000000000000`、writer bp `0x103a29b7c` (one-shot auto-continue) hit=0、`watchpoint list: No watchpoints currently set`；slot 值为 **0x0** 锁死分流方向——下一步进 **HOK-013**（在 PlayTools 里提前 touch Logger accessor `0x107e5df10` 预热 slot），不再去追 writer。`LaunchService.runLLDBHeadless` 的 abort-stop handler 两项固定规则：(A) slot 读必须显式 `memory read -fx -s 8 -c 1 <addr>`（不带 format 会与 LLDB 默认 byte-format 冲突、slot 值被静默丢弃）；(B) 尾部固定 `kill\nquit`（stop 现场已把证据采完，`continue` 会让 app 跑到 `-[NSWindow orderFront:]` 把 sheet 订上屏幕污染 b.0 gate）。`LaunchServiceTests` 28 个测试全过，`BuildScripts/build_and_install.sh Release` + ad-hoc 重签名成功。
- **当前已知事实**：
  ...
  - 结构化证据：`build/hok-007-ngr-callsite-report.json`、`build/hok-007b-ngr-patch-report.json`、`build/hok-011-ngr-common-init-chain-report.json`、`build/hok-011-analysis-notes.json`、`build/hok-012-ngr-watchpoint-report.json`、`build/hok-012-ngr-dyld-initializers.log`、`build/hok-012-c-3-b-0-dialog-detection-evidence.json`、`build/hok-012-c-3-b-3-report.json`（b.3 未污染 live：slot=`0x0`、writer bp hit=0）、`build/hok-012-c-3-b-3-dyld.log`（均在 `.gitignore` 的 `build/` 下，本地留存）。
- **当前主线**：`HOK-013`——在 PlayTools 里提前 touch Logger accessor `0x107e5df10`（或其 caller 的已知跳板），在 NGR 自身 reader 执行 `ldr x0, [0x10e2146f8]` **之前**把 slot 写入一个有效指针，绕过"slot 零填 → reader 拿到 null → `ldr x0, [x19]` 触发 deref"的整条通路。候选 E 继续作为**症状 workaround** 保留直到 HOK-013 落地；HOK-013 落地且验证闭环稳定后可以把候选 E revert 出来做反向对照。**HOK-013 的设计约束**：bundle-scoped（仅 `com.tencent.ngr`）；只 touch 一次；touch 路径不依赖 `__cxa_guard` 的 NGR 内部状态（直接从 PlayTools 侧写 slot 更干净）。具体写入点和写入值在 HOK-013 子文档做方案分析，不在 Dashboard 展开。
- **当前卡点**：无。HOK-013 的设计空间（写入时机、写入值、slide 处理）需要 HOK-013 子文档先做方案分析，但这是推进工作量而不是阻塞。
- **下一步默认规划**：
  ...
  2. `HOK-012-C.3-b.4`（保留）：只有 HOK-013 意外无法落地、或 slot 被 HOK-013 写进去之后 reader 链在更下游又触发新的 null deref，才回到"多 writer bp fallback / 沿坏值反查"这一条路。
  3. `HOK-012-C.3-b.5`（保留）：只有 HOK-013 落地后 reader 路径仍与 HOK-011 静态结论冲突，才回到 `Scripts/hok011_ngr_common_init_chain.py` 放宽扫描。
...
- **`0x10e2146f8` 在 live 下 slot 值为 `0x0`**（b.3 在 abort 现场实测，配合 writer bp `0x103a29b7c` hit=0 与 `watchpoint list: No watchpoints currently set`）：reader 读到 null 是 dyld 零填的直接后果，writer 在 LLDB 捕获窗口内从未被调用，分流唯一方向是 **HOK-013**（PlayTools 侧预热 slot）；b.4/b.5 仅作为 HOK-013 失败或新 null deref 时的兜底路线保留。
