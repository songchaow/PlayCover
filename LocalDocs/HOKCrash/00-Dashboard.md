...
- **当前结论**：`HOK-004`、`HOK-005A`–`HOK-005D`、`HOK-006`、`HOK-007A`、`HOK-007B`、`HOK-011`、`HOK-012-A`、`HOK-012-B`、`HOK-012-C.1`（legacy baseline）、`HOK-012-C.2`（pre-run watchpoint live run）、`HOK-012-C.3-a`（deferred-install watchpoint 自动化）、`HOK-012-C.3-b.1`（SIGABRT 拦截 + abort-stop handler 四层自动化）、`HOK-012-C.3-b.2`（`runLLDBHeadless` 收尾窗口参数化为 `teardownTimeoutSeconds`）、`HOK-012-C.3-b.0`（modal 对话框检测 + 残留进程强杀 + 污染 gate）均已完成。**重要事实修正**：`com.tencent.ngr` 启动后会**自主弹出一个 modal 对话框**（结构是普通 NSWindow + 自管 modal loop，**不是 `NSAlert`、`windowLayer=0`、没有 Apple CrashReporter 介入、不生成 `.ips`**），对话框阻塞 UI 线程后 app 其他线程继续跑 `SceneExtension hook_frame` 空转——看起来像"app 健康 running"、其实是被对话框钉死。**此前 b.3 的"live 证据"（slot=`0x10e23f200`、writer bp hit=0、backtrace 穿过 writer 函数体）全部是在对话框已弹出、app 已被冻结之后取到的，结论必须撤销**；对话框不拦掉则 LLDB 取到的任何 slot/bp/watchpoint 值都是冻结状态快照，不是真实 live 行为。b.0 现已把"检测 + 强杀 + 污染标记"写进四层自动化：host 侧 `LaunchService.runLLDBHeadless` 在返回 evidence 前调用 `CGWindowListCopyWindowInfo(.optionOnScreenOnly|.excludeDesktopElements, kCGNullWindowID)` + `/bin/ps -axo pid=,ppid=` 枚举子进程孙进程可见窗口、`kill(pid, SIGKILL)` 强杀所有残留，并把 `blockingDialogWindows`（结构含 `ownerPID`/`ownerName`/`windowName`/`windowLayer`/`alpha`/`isOnscreen`/`boundsX|Y|Width|Height`）与 `residualProcessesKilled` 写进 `LLDBLaunchEvidence`；`launch_app_with_lldb` JSON response 透传；`hok006_ngr_lldb_runner.py` `summarize_lldb_evidence` / `evaluate_lldb_capture` 消费，追加 `lldbBlockingDialogDetected` / `lldbResidualProcessesKilled` 两个 checks，`determine_overall_pass` 把"`blockingDialogDetected=true` → `overallPass=False`"作为 hard-fail gate；`LaunchServiceTests` 新增 2 个用例（`testLLDBLaunchEvidenceCarriesDialogAndResidualKillFields` / `testParseLLDBEvidenceForwardsDialogAndResidualKillInputs`），714 tests all passed；`BuildScripts/build_and_install.sh Release` + ad-hoc 重签名成功。**b.0 端到端 live 验证**：一轮 30s capture 的 summary = `blockingDialogs=2 residualPIDsKilled=1 overallPass=False`，2 个窗口都由 `ownerName=王者荣耀世界 / ownerPID=99561` 拥有（一个 260×204 小对话框 + 一个 1478×859 主窗口），强杀后 `ps -ax | grep NGR` 空输出、证据归档到 `build/hok-012-c-3-b-0-dialog-detection-evidence.json`。主线前移到：**重做 `HOK-012-C.3-b.3`（live slot/bp/watchpoint 证据采集）但必须先让 app 在对话框弹出之前被 LLDB 停住**；此前 b.3 的"DONE"标记已撤销、回滚到 TODO。
- **当前已知事实**：
  ...
  - 结构化证据：`build/hok-007-ngr-callsite-report.json`、`build/hok-007b-ngr-patch-report.json`、`build/hok-011-ngr-common-init-chain-report.json`、`build/hok-011-analysis-notes.json`、`build/hok-012-ngr-watchpoint-report.json`、`build/hok-012-ngr-dyld-initializers.log`、`build/hok-012-c-3-b-0-dialog-detection-evidence.json`（均在 `.gitignore` 的 `build/` 下，本地留存）。
- **当前主线**：`HOK-012-C.3-b.3-redo`——**重做** b.3 的 live run 采集，但必须先确保"LLDB 在对话框弹出之前就把 app 停下来"，否则取到的证据又是冻结状态快照。默认方案：在 `preRunCommands` 里追加 `breakpoint set --name "-[NSApplication runModalForWindow:]"` + `breakpoint set --name "-[NSWindow orderFront:]"`（后者作为 fallback 用于检测普通 NSWindow 被 order front 时做 modal 的情况）+ `breakpoint set --name "CGSOrderWindow"`（CoreGraphics 层 fallback）；以上任一 bp 命中就相当于对话框即将弹出、LLDB 立刻接管现场。单轮 live run 结束后同时检查 `checks.lldbBlockingDialogDetected` 必须为 False；如果还是 True，说明拦截的 bp 漏了，需要加 symbol。
- **当前卡点**：
  - 对话框的具体弹出路径还没识别：它不是 `NSAlert`（`windowLayer=0`），更可能是自绘 `NSWindow` + 自己跑 event loop 或调 `[NSApplication runModalForWindow:]`。b.3-redo 的 bp 列表必须覆盖这几条候选路径；必要时用 `target symbols list -n <partial>` 在 LLDB 里真列出 NGR 自己的 modal 函数 symbol（NGR 有大量 `___lldb_unnamed_symbol…`，可能需要补符号或按字符串反查）。
  - b.0 的窗口检测以"属于 NGR 进程 + onscreen + alpha≥0.05"为唯一判据；这会把 app 的主窗口也算成"blocking"（如本轮实测 2 个窗口里有 1 个是主窗 1478×859）。对于 HOK-012 场景"**任何** NGR 窗口留在屏幕上" 本身就是非预期（app 已经应该崩了或被 LLDB 停住），所以当前判据就是最保守正确的；不要过度分类。
- **下一步默认规划**：
  1. `HOK-012-C.3-b.3-redo`（当前主线，上面已列）。
  2. `HOK-012-C.3-b.4`：根据 b.3-redo 的 `memory read 0x10e2146f8` 结果分流：
     ...
     - slot 值为 **非 0 指针**：writer 路径跑过但我们没拦到；扩 writer bp 为多地址 fallback。
     - slot 值是个 **坏指针**：slot 被某个 bug 写坏了；沿值反查 writer。
  3. `HOK-012-C.3-b.5`：仅当 b.3-redo/b.4 仍无法在 **未污染** stop 上稳定取得 `0x10e2146f8` 值或与 HOK-011 静态结论冲突，才回到 `Scripts/hok011_ngr_common_init_chain.py` 放宽 store 形式并扫非 `__TEXT,__text` 段调用边。
...
| HOK-012-C.3-b.3 | TODO | **前置 b.0 gate 现已落地**；**b.3 必须先让 LLDB 在对话框弹出之前停住 app**，否则 `blockingDialogDetected=true` 直接 `overallPass=False`。默认方案：在 `preRunCommands` 里追加 `-[NSApplication runModalForWindow:]` / `-[NSWindow orderFront:]` / `CGSOrderWindow` 三条 bp；命中即触发 b.1 的 abort-stop handler 一次性采集 `memory read 0x10e2146f8` / `breakpoint list` / `watchpoint list`。命令仍是 `python3 Scripts/hok006_ngr_lldb_runner.py --watch-address 0x10e2146f8 --watch-size 8 --defer-watchpoint-install --lldb-timeout 60 --settle-seconds 65 --skip-build-install` + 新的 `--pre-run-command` 注入。必选观察：`lldbBlockingDialogDetected=False`、任一 bp 命中、transcript 尾段出现 slot 读取结果 | `LocalDocs/HOKCrash/HOK-012-C-watchpoint-live-trace.md`（"为什么 b.3 不能直接跑" + 对话框拦截 bp 列表） |
| HOK-012-C.3-b.4 | TODO | 按 b.3 (redo) 的 `memory read 0x10e2146f8` 结果分流：slot=0 → HOK-013（PlayTools 预热 slot）；slot=非 0 指针 → 扩 `--writer-addresses` 为 `0x103a29b7c,0x107e5df10` 多地址 fallback；slot=坏指针 → 沿坏值反查 writer。**分流判断只在 `lldbBlockingDialogDetected=False` 的 run 上做，否则证据无效** | `LocalDocs/HOKCrash/HOK-012-C-watchpoint-live-trace.md`（"deferred-install 模式下 bp 未命中 的正确读法" 6 步） |
| HOK-012-C.3-b.5 | TODO | 仅当 b.3/b.4 在未污染的 run 上仍无法取到可解释的 `0x10e2146f8` 值或与 HOK-011 静态结论冲突，才回到 `Scripts/hok011_ngr_common_init_chain.py` 放宽 store 形式（`str` / `stp` / `sturh` / ARM64 memcpy helper 间接 store）并扫描非 `__TEXT,__text` 段的 `bl 0x107e5df10` / `bl 0x103a29b7c` 调用边 | `LocalDocs/HOKCrash/HOK-011-静态初始化链分析.md` |
...
- **NGR 启动后会弹出一个自管 modal 对话框**（结构是普通 `NSWindow` + 自己跑的 modal event loop，`windowLayer=0` 而非 `NSModalPanelWindowLevel=8`，**不是 `NSAlert`、不生成 `.ips`、Apple CrashReporter 不介入**），弹出后阻塞 UI 线程，app 其他线程（包括 `SceneExtension hook_frame`）会继续跑但永不前进——看起来像"app 健康 running"、实际已冻结。在对话框弹出之后 LLDB 取到的任何 slot / bp hit count / watchpoint list 数据都是**冻结状态快照**，不能作为 live 行为证据。所有 HOK-012 的 watchpoint 类 live run **必须先保证对话框被 LLDB 在弹出前拦住**，或至少在结束时做 b.0 的对话框检测 + 污染 gate；污染 run 的"结论"必须一律作废、不能进入 Dashboard。
- **b.0 的对话框检测能力**（`LaunchService.runLLDBHeadless` → `CGWindowListCopyWindowInfo` + `/bin/ps -axo pid=,ppid=` 子进程孙进程枚举 + `kill SIGKILL`）是所有 live run 的**强制收尾步骤**，不论是否进入 watchpoint 模式。`evidence.blockingDialogWindows.isEmpty == false` 时 `checks.overallPass=False` 为 hard-fail；调用者看到 `blockingDialogs > 0` 必须把本轮证据视为 invalid、不能解读 slot / bp / watchpoint 的任何值。Python runner 的 summary 行含 `blockingDialogs=… residualPIDsKilled=…` 字段可一眼识别污染。
