...
**`SV-004`：在 `SV-006` 已明确升级边界的前提下，只围绕跨机器硬默认 gate 里的活跃候选维护最小行为证据；当前默认执行面已经收缩为 `test_fast_math_select = pass`、`test_intrinsic_vector_icmp_zext = deferred`，下一步只需要继续收口这个 deferred fragment 候选是否值得进入 automation-first 的 render-second。**
...
2. **让 `SV-004` 继续围绕收缩后的首批证据推进：不重开 `test_casts`，只维护剩余活跃候选的 compute-first / render-second 边界**
...
- 同一份代表产物里，`layeredDecision.overallDecision = promote_l2_candidates_to_l3` 保持不变，`l3Plan.candidateSampleKeys` 当前收缩为 `test_fast_math_select`、`test_intrinsic_vector_icmp_zext`
- `gate-summary.json` 当前已把 `test_casts` 记为 `resolvedL2SampleKeys`，说明它不再属于活跃 `L2` debt，而是本轮已收口的已知改进
- `behavior-summary.json` 默认直接写回 `build/semantics-validation/roundtrip/test-data-representatives/`：compute-first 只实际执行 `test_fast_math_select`，并将 `test_intrinsic_vector_icmp_zext` 结构化后置到 render-second
...
因此当前最高优先级已经进一步收窄为：**在不扩大人工成本、不引入新默认依赖的前提下，决定 `test_intrinsic_vector_icmp_zext` 这条 fragment deferred 证据是否值得升级到 automation-first 的 render-second；若做不到单命令、本地、无 UI、无工作区外修改，就继续保持 deferred，而不是为了覆盖率去抢跑 `SV-005`。**
...
- 若未来尝试为 fragment 候选补 render-second，也必须先满足“offscreen、单命令、本地、无 UI、无工作区外修改”；做不到这点时，文档应明确继续 deferred，而不是默认升级到 live
...
| SV-004 | 最小行为测试（compute-first） | ONGOING | P0（主线集合） | **当前主线。** 首版 compute-only harness 已能直接复用 `layeredDecision.l3Plan.candidateSampleKeys` 生成 `behavior-summary.json`；本轮已把 `test_casts` 从“行为 fail + 活跃 L2 候选”收口为“已修复并完成定向复核”，当前默认执行面已收缩为 `test_fast_math_select = pass` 与 `test_intrinsic_vector_icmp_zext = deferred` | `05-L3-最小行为测试.md` |
...
| SV-004D | 收口剩余 fragment 候选的 render-second 准入契约 | TODO | P0（当前执行面） | 只针对 `test_intrinsic_vector_icmp_zext` 这类仍然 deferred 的 fragment 候选，明确它是否值得进入最小 offscreen render-second；若无法保持“单命令、本地、无 UI、无工作区外修改”，则继续留在 deferred，不得因为追求覆盖率把 live / 人工依赖重新写回默认流程 | `05-L3-最小行为测试.md` |
...
- **首版行为级 oracle 已落地，且首个 fail evidence 已完成收口**：`test_fast_math_select` 已通过 reference-vs-generated 对跑，`test_casts` 已通过修复 `IRToMSLConverter` 的 `air.convert` unsigned 语义恢复逻辑完成收敛；`SV-004` 当前剩余的真实未决点只剩 `test_intrinsic_vector_icmp_zext` 这条 fragment deferred 证据应不应该进入 automation-first 的 render-second
