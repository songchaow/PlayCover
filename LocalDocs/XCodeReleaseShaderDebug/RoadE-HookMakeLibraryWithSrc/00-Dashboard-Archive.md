...
| 2026-04-06，`E-006d8` capture status probe decoupling | 针对 `on-run7` 观察到的 `get_capture_status -> Receive timed out`，定位到 status probe 自身承担了两层不当副作用：①强制 `valueOnMainSync` 占用主线程；②触发 `ensureGPUToolsCaptureLoaded()` / `dlopen(libmtlcapture)`。修复为：runtime 侧 `BridgeListener` 不再为主线程强制同步，`MetalCaptureService.getStatus(allowLazyLoad: false)` 不再在 probe 路径触发 lazy load；状态查询改为线程安全快照，`diagnostic_summary` 显式带出 `gpuToolsCaptureLoaded=`。**修复已落地但尚未经 fresh run 验证**；若仍 timeout，blocker 将进一步收敛到 capture command 路径本身 | blocker #1 排查面进一步收窄；区分"capture 库未加载"与"真正的 bridge / capture timeout"的能力已具备 |
| 2026-04-06，`E-006d8` fresh `replacement-on-run8` | `FORCE_PLAYTOOLS_REBUILD=1 ./BuildScripts/build_and_install.sh` 后 fresh `prepare-run(on,8)`、`launch_app`、等待进入更稳定界面，再做 `get_capture_status` / `capture_metal_frame` / `finalize-run --latest-gputrace`。这轮现象分三段：①刚进入 app 时，`get_capture_status` 首次返回 `captureManagerAvailable=false / gpuToolsCaptureLoaded=false / supportsGPUTrace=false`；②加载期 `capture_metal_frame` 一度报 host 侧 bridge timeout；③等待后，同一 session 的 `get_capture_status` 恢复为 `available=true / supportsGPUTrace=true`，默认 `Captures/` 目录也最终出现 `capture_20260406_roadE_e006d8_onrun8.gputrace`，并被成功固化为 `replacement-on-run8`。自动检查结果为 `valid_msl=3/14 (1.6%)`，矩阵更新后 `mode=on missingAttemptWhileEnabledPairs=13` | 这轮 fresh run 说明 blocker #1 不再应表述为"status probe 稳定超时"；更精确的表述应改为"加载期 command bridge / capture command 时序波动"，并且 host 侧 timeout 不能再直接等价于 capture 真失败，因为 `.gputrace` 可能稍后仍会在默认容器落盘 |
| 2026-04-06，`E-006d8` fresh `replacement-on-run9` | 沿用现成安装与 `prepare-run(on,9)` 后，直接对 `list_sessions` 暴露出的 ready session 做 live 复测：`get_capture_status` 立即返回 `available=true / supportsGPUTrace=true / trackedCommandQueues=2`；把 `capture_metal_frame` 显式指向 `~/Library/Containers/com.miHoYo.Yuanshen/Data/Documents/Captures/capture_20260406_roadE_e006d8_onrun9_custom.gputrace` 后，capture 成功启动并落盘，随后被固化为 `replacement-on-run9`。自动检查结果为 `valid_msl=3/12 (1.4%)`。但同轮重新走 `create_session(bundleId, timeout=20)` 仍报 *"Runtime registered ... but command bridge was not reachable within 20s"*，而 `RuntimeLaunchDiagnostics` 继续显示 launch / registration 主链完整 | blocker #1 从"capture/status 命令是否整体不可达"进一步收窄到**host 侧 `create_session` reachability probe / ready-session 复用判定**；blocker #2 从"自定义路径全部被拒"修正为"容器内 custom path 已验证可用，容器外权限边界未明" |
