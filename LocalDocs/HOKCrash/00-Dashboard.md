...
- **当前结论**：`HOK-004`、`HOK-005A`–`HOK-005D`、`HOK-006`、`HOK-007A`、`HOK-007B`、`HOK-011`、`HOK-012-A`、`HOK-012-B`、`HOK-012-C.1`（legacy baseline）、`HOK-012-C.2`（pre-run watchpoint live run）、`HOK-012-C.3-a`（deferred-install watchpoint 自动化）、`HOK-012-C.3-b.1`（SIGABRT 拦截 + abort-stop handler 的四层自动化落地）、`HOK-012-C.3-b.2`（`runLLDBHeadless` 收尾窗口参数化为 `teardownTimeoutSeconds`）均已完成。当前对 `com.tencent.ngr` 的理解是：候选 E patch 仍 apply，原 `0x10480df08` faulting window 已被压成 no-op；app 能跑过 PlayTools compat 序列、`GCloudCore` / `PluginManager` / `CAppLifecycleLaunch` C++ ctor 与 `SceneExtension hook_frame` 循环，随后触发 UE4 fatal（先 `Line:34`，再 `QtsFileSystem Create failed.` / `ICU data directory was not discovered`），`abort()` 在之前会弹出 Apple 崩溃对话框阻塞 UI。HOK-011 的 "`0x10e2146f8` 唯一 store = `0x103a29f7c`，唯一 reach 入口 = `0x103a29b7c`" 静态结论**仍然成立**；HOK-012-C.3-b.1/b.2 已把 SIGABRT 拦截与 5–10s 收尾窗口写进四层自动化（host 侧 `LaunchService.runLLDBHeadless` 的 stop-handler 在 watchpoint 模式下分流 `watchpoint ` vs 非 watchpoint，对 abort 形态的 stop 发送 `thread backtrace all` + `memory read -s 8 -c 1 <watchAddress>` + `breakpoint list` + `watchpoint list` + `continue`；`LLDBRunOptions.teardownTimeoutSeconds` 默认 2.0 保持 legacy 行为；`launch_app_with_lldb` MCP schema 新增同名字段；`hok006_ngr_lldb_runner.py` 新增 `--intercept-sigabrt` / `--no-intercept-sigabrt`（defer 模式默认 ON）与 `--teardown-timeout`（defer 模式默认 6.0s），并把 `process handle -s true -n true -p false SIGABRT` 无条件预置到 `preRunCommands` 首条），`LaunchServiceTests` 27 个用例全过（新增 `testLLDBRunOptionsTeardownTimeoutIsCallerControllable`），`BuildScripts/build_and_install.sh` 构建+安装+ad-hoc 签名成功。主线停在 `HOK-012-C.3-b.3`：在真实 NGR 环境跑一轮 defer + SIGABRT-intercept live run，验证 abort 能被 LLDB 停住并一次性取到 `0x10e2146f8` 当前值、`0x103a29b7c` bp 的 hit count 与 watchpoint list。
- **当前已知事实**：
  ...
  - HOK-012-C.3-b.1/b.2 已落地：`runLLDBHeadless` 的 watchpoint-mode stop-handler 对每一次新 `stop reason =` 先用 `"watchpoint "` 字样做分流——watchpoint-hit 走 legacy `thread backtrace\nframe variable\ncontinue\n`、abort-style 走 `thread backtrace all\nframe variable\nmemory read -s 8 -c 1 <watchAddress>\nbreakpoint list\nwatchpoint list\ncontinue\n`；`LLDBRunOptions.teardownTimeoutSeconds`（默认 2.0）替换旧硬编码 2.0s 收尾窗口；`launch_app_with_lldb` MCP schema 补 `teardownTimeoutSeconds` 字段（接受 int/double）；`hok006_ngr_lldb_runner.py` 新增 `--intercept-sigabrt`（默认 `--defer-watchpoint-install` 时 ON）、`--no-intercept-sigabrt` 与 `--teardown-timeout`（默认 defer 模式 6.0s、其他 2.0s），并在 defer+intercept 时把 `process handle -s true -n true -p false SIGABRT` 预置到 `preRunCommands` 首条（writer bp 行之前）；报告 `configuration` 和 `watchpoint.configuration` 新增 `interceptSigabrt` / `teardownTimeoutSeconds` 字段；`request_timeout = max(lldb_timeout + teardown_timeout + 15s, 20s)` 以覆盖加长的收尾；`LaunchServiceTests` 27 个用例全过（新增 `testLLDBRunOptionsTeardownTimeoutIsCallerControllable` 覆盖默认值保持 2.0、自定义值透传、下限钳制）；`BuildScripts/build_and_install.sh Release` + ad-hoc 重签名全部成功。
  - 结构化证据：`build/hok-007-ngr-callsite-report.json`（HOK-007A 一致性）、`build/hok-007b-ngr-patch-report.json`（HOK-007B apply 状态）、`build/hok-011-ngr-common-init-chain-report.json`（HOK-011 扫描报告）、`build/hok-011-analysis-notes.json`（HOK-011 深度静态分析 hand-off）、`build/hok-012-ngr-watchpoint-report.json`（HOK-012-B watchpoint 命中清单，脚本 `--watch-address` 启用后生成，HOK-012-C.3-b 之后额外携带 `interceptSigabrt` / `teardownTimeoutSeconds` 字段）、`build/hok-012-ngr-dyld-initializers.log`（HOK-012-B 子进程 stderr 重定向，与上者同轮产生、可交叉对齐时间戳）。
- **当前主线**：`HOK-012-C.3-b.3`——用落地好的 SIGABRT-intercept + 参数化 teardown 跑 `python3 Scripts/hok006_ngr_lldb_runner.py --watch-address 0x10e2146f8 --watch-size 8 --defer-watchpoint-install --lldb-timeout 60 --settle-seconds 65 --skip-build-install`（其余使用默认值即可：`--intercept-sigabrt` 在 defer 模式下默认 ON；`--teardown-timeout` 默认 6.0s）。本轮**不应**再看到 Apple 崩溃对话框；transcript 必须出现 `stop reason = signal SIGABRT`，且 stop handler 自动下发的 `memory read 0x10e2146f8` / `breakpoint list` / `watchpoint list` 的回显均应落到 transcript 尾段。预期读法见"下一步默认规划" b.4 的四个分流。`build/hok-012-ngr-dyld-initializers.log` 与 `build/hok-006-ngr-lldb-report.json.launch.lldb.transcriptTail` 互为交叉证据。
- **当前卡点**：
  - HOK-012-C.3-b.3 live run 尚未在真实环境执行——b.1/b.2 的改动属于可独立完成的四层自动化，b.3 必须在有 NGR 安装的机器上跑一轮才能落地证据。本文档状态表在 b.3 真跑完之前不能翻到 DONE。
  - `QtsFileSystem Create failed.` 与 Thread #10 `0x1047862bc`（`address=0x30`）是 UE4 层 fatal，可能是 `0x10e2146f8` reader 下游的连锁反应，也可能是独立问题；HOK-012-C.3-b.3 在 abort 上 stop 住之后，通过查看 `0x10e2146f8` 当前值 + 检查 reader 链的 caller `0x10460e2c0` 下游是否指向 `QtsFileSystem` 代码路径即可判明。
- **下一步默认规划**：
  1. `HOK-012-C.3-b.3`（当前主线）：按"当前主线"一节的命令跑 live run。必选观察：(a) transcript 里出现 `stop reason = signal SIGABRT`；(b) abort stop 自动发的 `memory read -s 8 -c 1 0x10e2146f8` 给出当前值；(c) `breakpoint list` 给出 `0x103a29b7c` bp 的 hit count；(d) 若 watchpoint 曾装上，`watchpoint list` 给出命中次数。本步不需要用户做任何手工操作。
  2. `HOK-012-C.3-b.4`：根据 b.3 的 `memory read 0x10e2146f8` 结果分流：
  ...
  3. 若 b.3/b.4 跑完仍然无法在 abort stop 上稳定取得 `0x10e2146f8` 的值，或取到了但和 HOK-011 静态结论无法对齐，才回到 `Scripts/hok011_ngr_common_init_chain.py` 放宽 store 形式（`str` / `stp` / `sturh` / ARM64 memcpy helper 的间接 store）并扫描非 `__TEXT,__text` 段里的 `bl 0x107e5df10` / `bl 0x103a29b7c` 调用边。
...
| HOK-012-C.3-b.3 | TODO | 在真实 NGR 环境跑一轮 `python3 Scripts/hok006_ngr_lldb_runner.py --watch-address 0x10e2146f8 --watch-size 8 --defer-watchpoint-install --lldb-timeout 60 --settle-seconds 65 --skip-build-install`（其余使用默认：defer 下 `--intercept-sigabrt` 默认 ON、`--teardown-timeout` 默认 6.0s）。必选观察：transcript 出现 `stop reason = signal SIGABRT`、abort-stop handler 自动下发的 `memory read -s 8 -c 1 0x10e2146f8` / `breakpoint list`（`0x103a29b7c` bp 的 hit count）/ `watchpoint list` 结果都落到 transcript 尾段 | `LocalDocs/HOKCrash/HOK-012-C-watchpoint-live-trace.md`（"deferred-install 模式下 bp 未命中 的正确读法" 6 步） |
| HOK-012-C.3-b.4 | TODO | 按 b.3 的 `memory read 0x10e2146f8` 结果分流：slot=0 → HOK-013（PlayTools 预热 slot）；slot=非 0 指针 → 扩 `--writer-addresses` 为 `0x103a29b7c,0x107e5df10` 多地址 fallback；slot=坏指针 → 沿坏值反查 writer | `LocalDocs/HOKCrash/HOK-012-C-watchpoint-live-trace.md`（"deferred-install 模式下 bp 未命中 的正确读法" 6 步） |
| HOK-012-C.3-b.5 | TODO | 仅当 b.3/b.4 无法在 abort stop 上稳定取到 `0x10e2146f8` 值，或取到了但和 HOK-011 静态结论冲突，才回到 `Scripts/hok011_ngr_common_init_chain.py` 放宽 store 形式（`str` / `stp` / `sturh` / ARM64 memcpy helper 间接 store）并扫描非 `__TEXT,__text` 段的 `bl 0x107e5df10` / `bl 0x103a29b7c` 调用边 | `LocalDocs/HOKCrash/HOK-011-静态初始化链分析.md` |
