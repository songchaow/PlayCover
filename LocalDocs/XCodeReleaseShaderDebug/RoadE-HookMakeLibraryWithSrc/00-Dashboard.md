...
- **E-006d（当前最高优先级）**：调查“用 PlayCover 打开原神，在同一界面重复启动时，画面表现每次都不完全一样；mesh 不变，但局部渲染结果异常”的现象。当前**不能实锤是 shader 改坏**：因为原神是延迟管线，base pass 对比看起来也可能类似，异常也可能来自后处理、着色阶段，或更上层的 render pipeline 顺序 / 配置差异。最新 `off1/off2/on1/on2/on3/on4` 六轮快照继续稳定排除了 **captured corpus / 单模块 artifact 层面的随机漂移**，但**仍未形成“跨模式稳定不同”的 corpus / trace 级证据**；因此 `replacement=off / on` 仍是最低风险的对照口径，但不能再表述成“已经稳定证明差异存在”的结论。
- **当前最该做的事**：`E-006d8` 的第一轮 run matrix 已经够用，下一步**不是**继续机械补 run，也不是回头扩 lowering；而是把 fresh `replacement=on` 的自动化 live 链路重新稳定下来。本轮在上一轮 `LLVMDisassembler` host bridge **重试 + 防误回退到本地 `posix_spawn`** 的基础上，进一步对 `BridgeListener` 做了 **listener 保活 + 启动期自动重试 + 注册通道断线重注册**，并让 `LLVMDisassembler` 在 session pending 时先短等 `2s` 再决定是否回退；`FORCE_PLAYTOOLS_REBUILD=1 ./BuildScripts/sync_playtools_xcframework.sh` 与 `./BuildScripts/build_and_install.sh` 均已通过，说明构建/安装链路未被新改动打坏。随后对原神执行 fresh `remove_playtools / inject_playtools / launch_app / create_session(timeout=120)`，结果仍是 **app 进程存活、`list_sessions` 为空、MCP `create_session` 请求超时**，且最近 Yuanshen unified log 中仍未看到 `PlayTools` / `BridgeListener` 关键字。这说明 blocker 已不再只停留在“注册需要重试”这一层，而是进一步收敛到：**fresh 注入后的 runtime 很可能根本没有稳定跑到 `PlayCover.launch()` / `BridgeListener.start()` 路径**；与此同时，`55` 次失败 attempt 仍集中落为 `reasonCode=exception`，`detail="Failed to launch llvm-dis: Operation not permitted"`。在这两条线收敛前，不应继续扩 run、也不应把日常 gate 推回人工协助；只有先在 agent 可独立完成的流程内恢复**至少一轮 `replacement=on` 且带 `.gputrace + aggregate source` 的稳定样本**，矩阵才有资格继续回答“替换 vs 不替换”是否稳定不同。
...
| 当前 `E-006d8` 第一层矩阵结论（2026-04-05） | `off1/off2/on1/on2/on3/on4` 六轮快照已经足够证明：**同模式输入稳定**、共享 `moduleKey` 为 **91/91**、单模块 `.bc/.ll/.metal` 本体未出现语义级随机漂移；`replacement-on-run4` 与 `on3` 的 run 快照摘要保持一致，`build/e006d-run-matrix.json` 最新摘要为：`groups.off.pairSummary.allPairsInputStable=true`、`groups.on.pairSummary.allPairsInputStable=true`、`groups.on.pairSummary.allSnapshotStable=true`、`crossMode.offVsOnPairSummary.allPairsDifferent=false`、`differentPairCount=6` |
| 当前尚未证明的部分（2026-04-05） | 现阶段**仍未形成“跨模式稳定不同”的 corpus / trace 级证据**：fresh `replacement-on-run3/on4` 虽已恢复 aggregate source 且 artifact 基线稳定，但仍未拿到新的 `.gputrace`；因此当前还不能把结论提前收敛为“替换开启后一定稳定引入了最终差异” |
| 当前 replacement 证据基线（2026-04-05） | 标准 `BuildScripts/build_and_install.sh` + fresh `replacement-on-run3/on4` 已恢复 runtime replacement 命中证据：两轮快照摘要都为 `manifestLines=1636`、`modules=91`、`replacements=36`、`diagnostics=18`；manifest 统计仍为 `replacement_attempt=91`（`succeeded=36`、`failed=55`），说明当前已不再是“enabled 但没有 attempt/aggregate”，且本轮 `LLVMDisassembler` host bridge 收口未引入新的 artifact 回归 |
| 当前 blocker 与下一步（2026-04-05） | 真正的 blocker 仍收敛为两件事：1）fresh 原神 on-run 的 runtime 注册链路不稳定，当前已不再只依赖 unified log，而是新增了 `RuntimeLaunchDiagnostics/<bundleId>/launch-events.jsonl` + `Scripts/runtime_launch_diagnostics_summary.py` 来判断 fresh 注入后是否真的进入 `PlayCover.launch()` / `BridgeListener.start()`、是否起 listener、是否发起/建立/丢失 registration channel；2）`replacement-on-run3/on4` 中其余 `55` 次失败 attempt 仍落为 `reasonCode=exception`，`detail="Failed to launch llvm-dis: Operation not permitted"`，说明当前仍是**部分替换成功 + 部分 `llvm-dis` 权限失败**。下一步应优先用现有自动化路径做一轮 fresh `replacement=on` live 复测并读取 breadcrumb，再继续收敛这两条线，而不是新增需要人工介入的日常 gate；只有先恢复**至少一轮 `replacement=on` 且带 `.gputrace + aggregate source` 的稳定样本**，再继续回答“替换 vs 不替换”是否稳定不同 |
...
|  | 本轮已新增 `RuntimeLaunchDiagnostics/<bundleId>/launch-events.jsonl` 与 `Scripts/runtime_launch_diagnostics_summary.py`，并在 `PlayCover.launch()` / `BridgeListener.start()` / registration retry / disconnect 关键节点持久化 breadcrumb。下一步不再盲猜 unified log，而是先用 fresh `replacement=on` live 复测 + 该脚本确认：runtime 是否进入 `PlayCover.launch()`、是否起了 command listener、是否发起注册、是否建立或丢失 registration channel。与此同时，`55` 个 `reasonCode=exception` / `detail="Failed to launch llvm-dis: Operation not permitted"` 的失败 attempt 仍需并行收敛；在恢复至少一轮 `replacement=on` 且带 `.gputrace + aggregate source` 的稳定样本前，不应继续把结论过早推向 shader lowering 或更后续 pipeline，也不应把人工协助升级为日常前提。 | | |
