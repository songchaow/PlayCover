...
- **E-006**：`LLVMDisassembler` 的 runtime→host `llvm-dis` bridge、`makeLibrary(source:)` 前的 MSL preflight guard，以及 `E-006a2c1` 的 `Toucher.touchcam` 空值防护均已落地，并已通过 `FORCE_PLAYTOOLS_REBUILD=1 ./BuildScripts/sync_playtools_xcframework.sh`、`./BuildScripts/build_gui.sh`、`./BuildScripts/test_mcp.sh RegistrationListenerHostCommandTests` 验证构建与桥接链路。2026-04-03 基于这版构建完成了 `E-006a2c2` 的 **关闭 `keymapping` 后 2 轮受控 live 复测**：按 `PLAYCOVER_INSTALL_MODE=user FORCE_PLAYTOOLS_REBUILD=1 ./BuildScripts/build_and_install.sh` 重装 GUI，对原神设置 `keymapping=false`，并从 `pkill Yuanshen` 干净状态执行 `remove_playtools` / `inject_playtools` / `launch_app` / `create_session`。第 1 轮 `session` 在 `00:47:37 +0800` ready 后，前 10 秒仍保持 ready，约 **42 秒**后 app 退出并新增 `Yuanshen-2026-04-03-004819.ips`；第 2 轮 `session` 在 `00:50:00 +0800` ready 后约 **10 秒**即变为 `disconnected`，并新增 `Yuanshen-2026-04-03-005012.ips`。两轮新 crash 均不再落在 `playcover.toucher`，而是统一回到 `UnityGfxDeviceWorker -> objc_release -> objc_autoreleasePoolPop` 的 `EXC_BAD_ACCESS / SIGSEGV`；与此同时，`~/Library/Containers/io.playcover.PlayCover/ShaderSourceDiagnostics/com.miHoYo.Yuanshen/` 新增了多份 `preflight_rejected` / `compile_failed` 样本，最新 compile failure 仍为 `program_source:14:16: expected unqualified-id`，坏行示例是 `fragment float4{ <4 xlatMtlMain(...)`。此前已完成 `E-006a2d1` 的 packed-return / 默认 builtin 参数映射修复；本轮又把原 `E-006a2d2` 进一步拆成“`E-006a2d2a` 先修 metadata→参数映射 / suffixed SSA / fast-math 二元算术解析”“`E-006a2d2b` 再继续收 texture/sampler 形参发射与自定义 struct 字段命名/定义”两步，并已完成 **`E-006a2d2a`**：1) 修正 `parseMetadataFuncNode` 取参数列表引用时把 `ptr @func` 误算进 refs 的 off-by-one；2) `generateFunctionWithBody` 现改为把**纯 IR 参数列表**而不是整条 `define` 签名传给 `setupParameterMappings`；3) `resolveIROperand` 现可回接带限定词的 SSA 以及 `%1.xyz` / `%0.field3[0]` 这类 suffixed SSA；4) `translateBinaryFP/Int` 与 `parseBinaryOperands` 现正确处理 `fadd/fmul/fdiv fast` 和 `<type> lhs, rhs` 的共享类型写法。已新增 `test_fast_math_binary.metal/.ll`，并用离线 harness 对 `test_fast_math_binary.ll` 与 `test_extractvalue.ll` 复测：生成结果里已不再出现 `%...`、`<N x T>`、`fadd/fmul/fdiv fast` 或空 RHS；其中 `test_fast_math_binary` 的转换产物已通过 `xcrun --sdk macosx metal -c`，同时本轮仍通过 `FORCE_PLAYTOOLS_REBUILD=1 ./BuildScripts/sync_playtools_xcframework.sh`。这说明当前 preflight blocker 已从原先的 SSA / vector / fast-math token 泄漏，进一步收敛到 **texture/sampler 形参发射细节** 与 **自定义 struct 的字段命名 / 定义缺失**。
...
|  | runtime→host 的 `llvm-dis` bridge、聚合 MSL preflight guard，以及 keymapping 隔离 live 复测均已完成；本轮在 `E-006a2d1` 基础上继续落地了 `E-006a2d2a`，离线清掉了 `%...` / `<N x T>` / `fadd/fmul/fdiv fast` / 空 RHS 这批 preflight token 泄漏，并通过 toolchain + PlayTools 构建验证。下一步转向 `E-006a2d2b`，继续收 texture/sampler 形参发射与自定义 struct 字段命名/定义，再决定是否进入下一轮 live 截帧复测 | | |
...
| E-006a2d | ↳ 基于最新 diagnostics 样本修 IR→MSL 中的返回签名 / vector / SSA 泄漏 | 🔄 IN PROGRESS | |
|  | 已拆成“`E-006a2d1` 先修 fragment packed-return / 默认 builtin 参数映射”“`E-006a2d2` 再继续清 `%0.field*`、`<4 x float>`、`fadd/fmul fast` 等 LLVM token 泄漏”两步；其中 `E-006a2d2` 本轮又进一步拆成“`E-006a2d2a` 先修 metadata→参数映射 / suffixed SSA / fast-math 二元算术解析”“`E-006a2d2b` 再继续收 texture/sampler 形参发射与自定义 struct 字段命名/定义”两步，避免把 preflight token 泄漏和剩余可编译性问题继续混在一起 | | |
...
| E-006a2d2 | ↳ 继续修 vector / SSA / fast-math 泄漏 | 🔄 IN PROGRESS | |
|  | `E-006a2d2a` 完成后，`test_fast_math_binary.ll` 与 `test_extractvalue.ll` 的离线转换结果里已不再出现 `%...`、`<N x T>`、`fadd/fmul/fdiv fast` 或空 RHS；下一步转向 `E-006a2d2b`，继续修 texture/sampler 形参发射与自定义 struct 字段命名 / 定义，再决定是否进入下一轮 live 截帧复测 | | |
...
| E-006a2d2b | ↳ texture/sampler 形参发射 + 自定义 struct 字段命名/定义 | TODO | |
|  | 当前离线转换的剩余显著问题已从 preflight token 泄漏收敛到 `test_sample_extract(...)` 的 texture/sampler 形参发射异常，以及 `Uniforms` / `Particle` 这类自定义 struct 仍使用 `.fieldN` 但缺少匹配定义；补完后再评估是否进入下一轮 live 复测 | | |
...
- **metadata 过滤默认 builtin 参数后，SSA→MSL 参数映射也要同步补回**：像 fragment 的 `air.position` 可能不会进入 `ParsedParameter`，但函数体 IR 仍直接引用 `%0`；若 `setupParameterMappings` 不把它接回默认 builtin 名，函数头即使已恢复为 `position [[position]]`，函数体仍会继续泄漏未声明的 `param0`
