...
|  | runtime→host 的 `llvm-dis` bridge、聚合 MSL preflight guard、keymapping 隔离 live 复测，以及 `E-006a2d1/d2/d3` 的离线 IR→MSL 修复均已完成；当前 `test_extractvalue.ll` 已能全量生成并通过 `xcrun --sdk macosx metal -c`，下一步回到新的 live 截帧复测，确认 runtime / `.gputrace` 侧是否继续前移 blocker | | |
...
|  | 已完成 `E-006a2d1`、`E-006a2d2a`、`E-006a2d2b`、`E-006a2d3`，离线把 `%...` / `<N x T>` / `fadd/fmul/fdiv fast` / 空 RHS / 坏 texture-sampler 形参 / `.fieldN` 缺定义 / vertex aggregate return / 向量维度漂移 这几类 blocker 清掉；`test_extractvalue.ll` 现已恢复 `texture2d<float>`、`sampler`、fragment `stage_in`、`Uniforms.modelViewProjection`、`Particle.position/velocity`、`vertex Test_vertex_struct_Out` 与 `float3 * float3`。下一步转回新的 live 截帧复测，确认 runtime / `.gputrace` 侧是否还存在独立 blocker | | |
