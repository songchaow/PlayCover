...
|  | runtime→host 的 `llvm-dis` bridge、聚合 MSL preflight guard、keymapping 隔离 live 复测，以及 `E-006a2d1/d2/d3/d4` 的离线 IR→MSL 修复均已完成；当前离线产物已把 `param1/param2/param3` 与 `*(&...)` 这类 blocker 清掉，下一步转入 `E-006a2e` 做 live 重装 / 重注入 / diagnostics 复测 | | |
...
|  | 已拆成“`E-006a2a` 先拦截明显非法的聚合 MSL 并落盘诊断样本”“`E-006a2b` 基于新 guard 做 live 对照复测”“`E-006a2c` 先修 toucher 自身崩溃，再做 keymapping 隔离后的 live 复测”“`E-006a2d` 再基于最新 diagnostics 样本继续修 IR→MSL 产物”“`E-006a2e` 基于 `E-006a2d4` 做新一轮 live 复测”五步，避免把源码无效、输入路径崩溃和 capture 条件不足混在一起；截至本轮，`E-006a2d4` 已离线把 vertex `stage_in` / pointer 访问路径修正到可编译状态，下一步看 live diagnostics 是否同步前移 | | |
...
| E-006a2e | ↳ 基于 `E-006a2d4` 的 live 重装 / 重注入 / diagnostics 复测 | TODO | |
|  | 用 `PLAYCOVER_INSTALL_MODE=user FORCE_PLAYTOOLS_REBUILD=1 ./BuildScripts/build_and_install.sh` 重装 GUI，并对原神重新执行 `remove_playtools` / `inject_playtools` / `launch_app` / `create_session`；重点确认 `ShaderSourceDiagnostics` 中是否还残留 vertex `stage_in` 映射缺失、`device T*` 访问坏行或 `*(&...)` 模式，再决定是否进入 capture | | |
