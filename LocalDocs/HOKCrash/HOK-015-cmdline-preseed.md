...
> 已落地 / 下一步" 等字样，也不写日期快照。
>
> **阅读建议**：修改 HOK-015 相关代码时总是读取；日常 HOK-016 系列工作
> 不需要进入。
>
> **相关文档**：
>
...
四轮验证的完整命令 / 判据、与 HOK-014 / HOK-016 的联动归属规则、
bundle-scoped gate 回归检查、以及非目标 / 边界（不解决 UE4 主循环业务
错误、不改 cmdline 语义、不适用其它 UE4 app）一律沉淀到
`HOK-015-appendix-verification.md`。概要：

- **第一轮（HOK-015 专属判据）**：locator 扫出 slot → 写入 PlayLoader
  宏 → 自由启动 NGR，确认 `hok015_ngr_cmdline_preseed status=primed`、
  `bInitializedBefore=0 / After=1`、子进程 stderr 不再出现 UE4 cmdline
  fatal。
- **第二轮（与 HOK-016 联动）**：`hok014_ngr_alert_suppressed = 1` 且
  message 是 `"QtsFileSystem Create Failed!!"` 时归入 HOK-016，**不**
  视为 HOK-015 回归；只有 UE4 cmdline fatal 重现才算 HOK-015 回归。
- **第三轮（HOK-014 降级）**：仅在 HOK-016-D 通过后才允许把 Dashboard
  的 HOK-014 状态描述改为"DONE（冷备安全网）"。
- **第四轮（bundle gate）**：确认 `hok015_ngr_cmdline_preseed` 事件只
  出现在 `com.tencent.ngr` 的 `RuntimeLaunchDiagnostics/` 子目录里。

> **何时进附录**：要真正跑一轮 HOK-015 验证、判断一次 alert 归属是
> HOK-015 回归还是 HOK-016 残留、或考虑推广到其它 UE4 app 时读；日常
> 主文档浏览不必进入。

## 参考

- `HOK-015-appendix-verification.md`：四轮验证命令 / 判据、与 HOK-014
  / HOK-016 的联动规则、非目标与边界。
