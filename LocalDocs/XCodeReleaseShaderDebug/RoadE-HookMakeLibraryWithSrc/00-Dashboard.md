...
- **E-006**：`LLVMDisassembler` 的 runtime→host `llvm-dis` bridge、`makeLibrary(source:)` 前的 MSL preflight guard，以及 `E-006a2c1` 的 `Toucher.touchcam` 空值防护均已落地，并已通过 `FORCE_PLAYTOOLS_REBUILD=1 ./BuildScripts/sync_playtools_xcframework.sh`、`./BuildScripts/build_gui.sh`、`./BuildScripts/test_mcp.sh RegistrationListenerHostCommandTests` 验证构建与桥接链路。2026-04-03 先完成了 `E-006a2c2` 的 **关闭 `keymapping` 后 2 轮受控 live 复测**，确认 toucher / keymapping 干扰已基本从 shader 主线中剥离；随后又在 `E-006a2d1`、`E-006a2d2a`、`E-006a2d2b` 的 packed-return / 参数映射 / fast-math / texture-sampler / struct 定义修复基础上完成 **`E-006a2d3`**：1) `parseMetadataFuncNode` 现会同时解析 return metadata，`identifyShaderFunctions` / `generateMSL` 会为多字段 vertex/fragment 输出生成专用 entry output struct；2) `translateRet` 在返回匿名 aggregate 时会补上显式返回类型，避免继续发射 `return { t16, t17 };` 这类坏 MSL；3) `translateShuffleVector` / `parseVectorConstant` 现按 mask 类型维度处理 `zeroinitializer`，修复 `<3 x i32> zeroinitializer` 被误扩成 4 维导致的 `float4(...) * float3`。离线 harness 对 `test_extractvalue.ll` 的复测已恢复 `vertex Test_vertex_struct_Out ... return Test_vertex_struct_Out{ t16, t17 };` 与 `float3 * float3`，并通过 `xcrun --sdk macosx metal -c` 与 `FORCE_PLAYTOOLS_REBUILD=1 ./BuildScripts/sync_playtools_xcframework.sh`。基于这版构建，本轮又完成了 **`E-006a2d3` 后单轮 live 复测**：按 `PLAYCOVER_INSTALL_MODE=user FORCE_PLAYTOOLS_REBUILD=1 ./BuildScripts/build_and_install.sh` 重装 GUI，并从 `pkill Yuanshen` 干净状态执行 `remove_playtools` / `inject_playtools` / `launch_app` / `create_session(timeout=30)`。结果是 runtime 未在 30 秒内注册，app 在启动约 **3.4 秒**后即退出并新增 `Yuanshen-2026-04-03-103757.ips`；`ShaderSourceDiagnostics/com.miHoYo.Yuanshen/` 同时新增 `2026-04-03T02_37_56Z_newLibraryWithData_error__compile_failed.{metal,txt}`。新 compile failure 已不再是 `<N x T>` / `%...` 这类旧 blocker，而是聚合 vertex `xlatMtlMain` 里暴露出 `param1/param2/param3` 未映射、`device VGlobals_Type*` 仍按 `.` 而非 `->` 访问、以及 `*(&foo[1])` / `*(&foo)` 这类非法取址模式；新 crash 仍统一回到 worker thread 的 `objc_release -> AutoreleasePoolPage::releaseUntil -> objc_autoreleasePoolPop`。说明 `E-006a2d3` 已把旧的 SSA/vector blocker 清掉，当前主线已前移到 **vertex 参数映射 + pointer/aggregate 访问发射**。
...
|  | runtime→host 的 `llvm-dis` bridge、聚合 MSL preflight guard、keymapping 隔离 live 复测，以及 `E-006a2d1/d2/d3` 的离线 IR→MSL 修复均已完成；最新单轮 live 复测已把旧的 `<N x T>` / `%...` blocker 清掉，但新的 `compile_failed` 样本又把问题收敛到 vertex 参数映射与 pointer/aggregate 访问发射，下一步先做 `E-006a2d4` 再回到 live 截帧复测 | | |
...
|  | 已拆成“`E-006a2a` 先拦截明显非法的聚合 MSL 并落盘诊断样本”“`E-006a2b` 基于新 guard 做 live 对照复测”“`E-006a2c` 先修 toucher 自身崩溃，再做 keymapping 隔离后的 live 复测”“`E-006a2d` 再基于最新 diagnostics 样本继续修 IR→MSL 产物”四步，避免把源码无效、输入路径崩溃和 capture 条件不足混在一起；截至本轮，`E-006a2d3` 后的单轮 live 复测已确认最新 blocker 前移到 `xlatMtlMain` 的参数映射 / pointer 访问问题 | | |
...
| E-006a2d | ↳ 基于最新 diagnostics 样本继续修 IR→MSL 中的参数映射 / pointer / aggregate 访问 | 🔄 IN PROGRESS | |
|  | 已完成 `E-006a2d1`、`E-006a2d2a`、`E-006a2d2b`、`E-006a2d3`，离线把 `%...` / `<N x T>` / `fadd/fmul/fdiv fast` / 空 RHS / 坏 texture-sampler 形参 / `.fieldN` 缺定义 / vertex aggregate return / 向量维度漂移 这几类 blocker 清掉；随后在 `E-006a2d3` 后完成了单轮 live 复测，最新 `compile_failed` 样本已把剩余问题收敛到 vertex `xlatMtlMain` 的 `param1/param2/param3` 未映射、`device VGlobals_Type*` 仍按 `.` 而非 `->` 访问，以及 `*(&foo[1])` / `*(&foo)` 这类非法取址模式。下一步转入 `E-006a2d4` | | |
...
| E-006a2d4 | ↳ vertex 参数映射 + pointer/aggregate 访问发射修复 | TODO | |
|  | 目标是让 `xlatMtlMain` 这类最新 live 样本恢复到合法的 vertex 输入形参、`VGlobals->...` / 合法下标访问，以及去掉 `*(&...)` 伪解引用，再做下一轮 live 复测 | | |
...
- **参数映射不能只按过滤后的 `ParsedParameter` 顺序对齐**：像 fragment 同时带 `air.position`、`air.fragment_input`、`air.texture`、`air.sampler` 时，若 `setupParameterMappings` 仅按“过滤后剩余参数”的顺序去对齐 IR `%0..%N`，很容易把 `%2/%3` 错接成 `smp/param2`。更稳的做法是保留 metadata `argIndex`，按原始 IR 参数位次精确回接；若存在 `air.fragment_input`，则把 `air.position` + varying 一起合成为专用 `stage_in` struct，再把 `%0/%1/...` 映射到 `stageIn.xxx`
...
- **`device T*` 的 aggregate 访问不能继续走“`.` + `*(&...)`”退化路径**：最新 `xlatMtlMain` 样本里，`device VGlobals_Type* VGlobals` 若仍按值对象发射成 `VGlobals.field`，再叠加通用的取址/解引用模板，就会稳定生成 `VGlobals.hlslcc_mtx4x4unity_ObjectToWorld[1]`、`*(&VGlobals._MainTex_ST)` 这类非法 MSL；下一步必须按 pointee/container 形态区分 `->field`、合法下标访问和直接值传递
