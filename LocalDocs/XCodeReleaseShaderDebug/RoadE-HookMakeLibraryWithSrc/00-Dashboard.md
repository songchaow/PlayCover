...
- **E-006（本轮执行：离线 compile blocker 收敛——补齐 metadata 缺失但被函数体真实引用的 IR 值参数）**：`IRToMSLConverter` 现在会在 metadata 路径与 metadata 缺失路径下，保守补齐“函数体真实引用到、但签名里缺失”的普通 IR 值参数，同时继续把默认 entry builtin（`position` / `vid` / `tid`）保留为 builtin，而不是误降级成普通参数；对 `test-data/*.ll` 的 baseline compare 结果更新为 replay `18/18` 成功、Metal compile `11/18` 成功、compile regression `0`、improvement `1`。其中 `test_sample_compare_depth_2d.ll` 已从 `undeclared_identifier:param2` 提升为编译通过，原先最显著的 `undeclared_identifier` failure cluster 已从当前最小样本基线中消失；`test_metal_intrinsic_sampler_state.ll` 则前移暴露为 `invalid type 'float2' ... in a kernel function`，说明下一步应优先收敛 metadata / shader type / 资源种类识别，而不是继续修同一类“缺参未声明”问题。
...
| 当前最小离线验证基线（2026-04-04） | 已对 `test-data/*.ll` 执行一轮新的 batch replay + compile + baseline compare：replay `18/18` 成功，Metal compile `11/18` 成功；对 `build/road-e-e005c-verify/baseline-snapshot/baseline.json` 的 compare 结果为 matched/new/removed `18/0/0`、replay changed `0`、generated MSL changed `2`、compile changed `2`，其中 regression `0`、improvement `1`。当前 `test_sample_compare_depth_2d.ll` 已从 `undeclared_identifier:param2` 提升为 compile success，`undeclared_identifier` failure cluster 已不再出现在最新最小样本基线中；剩余 blocker 之一是 `test_metal_intrinsic_sampler_state.ll` 的 shader type / 资源识别仍不足 |
...
> 当前最高优先级：`E-006c`（真实 `.gputrace` 源码可见确认）。在 live 稳定前，继续优先收敛 IR→MSL lowering / compile blocker；本轮已先完成一轮“缺失值参数未声明”类 blocker 收敛，下一步应转向 metadata / shader type / resource kind 识别不足导致的 compile blocker。历史 live blocker 归因链路见 [00-Dashboard-Archive](00-Dashboard-Archive.md)。
...
| E-006b2 | ↳ 收敛 metadata / shader type / resource kind 识别不足的 compile blocker | TODO | |
|  | 当前代表样本为 `test_metal_intrinsic_sampler_state.ll`：`undeclared_identifier` 已被前移消除，但函数仍被误判为 kernel，且 `ptr addrspace(1)` / `ptr addrspace(2)` 资源种类仍未恢复为 texture / sampler | | |
