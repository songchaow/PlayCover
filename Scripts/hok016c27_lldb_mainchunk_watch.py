...
def snapshot_create_table_impl_materialize_precall_on_hit(frame, bp_loc, internal_dict):
    thread = frame.GetThread()
    process = thread.GetProcess()
    ...
    helper_vtable_norm = _normalize_ngr_ptr(helper_vtable)
    helper_slot10 = _read_u64(process, x19 + 0x10) if x19 > 0x100000000 else None
    helper_slot18 = _read_u64(process, x19 + 0x18) if x19 > 0x100000000 else None
    helper_slot28 = _read_u64(process, x19 + 0x28) if x19 > 0x100000000 else None
    ...
    _log(
        ...
        f"x8.norm=0x{(target_norm or 0):x} helper.vtable=0x{(helper_vtable or 0):x} helper.vtable.norm=0x{(helper_vtable_norm or 0):x} "
        f"helper+0x10=0x{(helper_slot10 or 0):x} helper+0x18=0x{(helper_slot18 or 0):x} helper+0x28=0x{(helper_slot28 or 0):x}"
        f"{_pointer_raw_extra(process, 'x0', x0)}{_pointer_raw_extra(process, 'x1', x1)}{_pointer_raw_extra(process, 'x8', x8, 0x10)}"
        f"{_pointer_raw_extra(process, 'x21', x21)}{_pointer_raw_extra(process, 'x22', x22)}"
        f"{_pointer_raw_extra(process, 'helper_slot10', helper_slot10)}{_pointer_raw_extra(process, 'helper_slot28', helper_slot28)}"
        f"{_watched_err_slot_extra(process)} bt={_short_backtrace(thread)}"
    )
    return False
