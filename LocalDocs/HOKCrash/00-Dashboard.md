...
- **当前结论**：`HOK-004`、`HOK-005A`、`HOK-005B`、`HOK-005C`、`HOK-005D`、`HOK-006`、`HOK-007A`、`HOK-007B`、`HOK-011`（静态分析阶段）、`HOK-012-A`（bundle-scoped `DYLD_PRINT_INITIALIZERS=1` 注入）、`HOK-012-B`（pre-run watchpoint + dyld-log 自动化）、`HOK-012-C.1`（legacy 基线 live 确认候选 E 有效）、`HOK-012-C.2`（首轮 pre-run watchpoint live run，`0x10e2146f8` 0 hit + secondary fault 指向 HOK-010）、`HOK-012-C.3`（deferred-install watchpoint：LLDBRunOptions.deferWatchpointInstall 贯穿 Swift/LLDB/MCP/Python 四层 + `--writer-address` 默认 `0x103a29b7c` 的 writer-entry bp 自动注入，live 证明 writer 函数在 5s/20s 内均未被命中）均已完成。C.3 的 live 证据**直接证伪了 HOK-011 的核心假设**：既然 `0x103a29b7c` 是 `0x10e2146f8` 唯一 store 的所在函数（静态证据），而该函数在 app 启动 20s 内（已跑完 PlayTools compat 序列、GCloudCore/PluginManager/CAppLifecycleLaunch C++ ctor、`SceneExtension hook_frame` 循环，时间窗跨过 C.2 里出现 `QtsFileSystem Create failed.` 的阶段）**从未被调用**，那么在 live 下该 slot 的值要么**根本没被写入**（reader 读到的就是 dyld 零填），要么由一条**根本不经过 `0x103a29b7c`** 的路径写入（这与 HOK-011 全二进制 BFS 的 "只有 `0x103a29f7c` 一处 store" 结论冲突，意味着动态 patch / 外部 dylib 的间接写入是唯一可能，超出本主线可继续追踪的范围）。综合 C.1/C.2/C.3 三轮 live 与 HOK-011 静态分析：`0x10e2146f8` 这条 root cause 追查已走到收益边缘，候选 E 在 app 跑到 `QtsFileSystem` 阶段的事实说明它的 workaround 语义对 startup 期是充分的；`HOK-012` 整体可以安全 DEFER，主线应重新回到 **HOK-010（`rootWorkDir` / `QtsFileSystem Create failed.`）** 与 **HOK-007C（`0x1047862bc` 下游 fault 的 HOK-007A 风格离线映射）**。HOK-012-A/B/C 的自动化栈保持不变且可立即复用：host 侧（`PlayApp` / `LaunchService` 两处 `effectiveLaunchEnvironment`）的 bundle-scoped `DYLD_PRINT_INITIALIZERS=1` / `DYLD_PRINT_APIS=0` 注入、headless LLDB runner + `launch_app_with_lldb` MCP 工具 + `hok006_ngr_lldb_runner.py` 脚本四层同步扩展的 watchpoint 自动化（`watchAddress`/`watchSize`/`preRunCommands`/`dyldInitializersLogPath`/`deferWatchpointInstall`）、`process launch -e <dyldInitializersLogPath>` 子进程 stderr 重定向、`stop 抓 backtrace 后 continue` 的多命中循环、deferred-install 模式下 Python runner 的 `--writer-address` 自动 bp 注入，legacy 行为通过 `LLDBRunOptions.default` / 缺省 MCP 字段保持 100% 兼容，`LaunchServiceTests` 26 个用例全部通过，`BuildScripts/build_and_install.sh` 构建 + 安装 + ad-hoc 签名成功。
- **当前已知事实**：
...
- **当前主线**：`HOK-007C`（下游 fault 离线映射 + 最小可逆 patch）。C.3 结论已经把 `0x10e2146f8` 的 root-cause 追查推到收益边缘，下一步价值最高的是对 C.2 live 观察到的 secondary fault（Thread #10，`0x1047862bc` + `address=0x30`，`ldr w9, [x8, #0x30]`）做 HOK-007A 风格的离线 callsite 一致性映射，并视结果决定是在二进制上再落一个 HOK-007B 风格的最小可逆 patch，还是把注意力全部切到 HOK-010（`rootWorkDir` / `QtsFileSystem`）。同步待起动的 `HOK-010` 方向是把 `rootWorkDir` 从 `PlaySettings.disableForMinimalStartupCompat(...)` 摘除，观察 `QtsFileSystem Create failed.` 是否消失——这条线与 HOK-007C 是正交的，可以并行推进。`HOK-012` 整体 DEFER，但 `HOK-012-A/B/C` 就位的 watchpoint / dyld-log / deferred-install 自动化栈不拆除，作为后续任何新 slot 追查的默认工具。
- **当前卡点**：
  - HOK-012 整体 DEFER，`0x10e2146f8` 的真实 writer 已证伪落在 `0x103a29b7c`——live 20s 没命中这个函数，与 HOK-011 静态"唯一 store = `0x103a29f7c`"冲突，说明要么 slot 根本没被写（reader 读到 0 仍能被 candidate-E 压下去）、要么存在 HOK-011 BFS 视角之外的间接写入；无论哪种，继续追 HOK-012 都无法靠本仓内可控手段拿到下一条硬证据。
  - HOK-007C 的 live 证据（C.2 里 Thread #10 的 `0x1047862bc` secondary fault）是**非主线程 + NSThread start** 产生，backtrace 深度 254；需要一次单独的 headless LLDB 复现，把它的 `__TEXT,__text` file offset 和 `.ips` `usedImage.base + imageOffset` 交叉校验，沿用 HOK-007A 的口径落到一份 `build/hok-007c-ngr-callsite-report.json`。
  - HOK-010 live 已在 C.2 复现 `QtsFileSystem Create failed.` + `ICU data directory was not discovered`；重启它之前要先在离线层确认"摘除 `rootWorkDir` 强制关闭"不会让 `cwd` 回到容器外而绕过 sandbox——`PlaySettings.disableForMinimalStartupCompat` 的 call site 仍要求 bundle-scoped 语义。
  - `launch_app_with_lldb` 的 deferred-install 模式在 `--lldb-timeout 20` 下仍只能覆盖 `SceneExtension hook_frame` 循环之前的启动段；若未来需要观察 C.2 里 `QtsFileSystem Create failed.` 发生时刻的精确上下文，必须把 MCP 端 request timeout 与 `runLLDBHeadless` 的超时以及 `process interrupt` 收尾窗口（当前固定 2s）三者一起拉大；当前实现以"5s 默认"为口径对 HOK-007C / HOK-010 足够，但给 HOK-012 复活预留了扩容口径。
- **下一步默认规划**：
  1. `HOK-007C.1`：复用 `launch_app_with_lldb`（legacy 模式）跑一轮 `com.tencent.ngr`，用默认 `--lldb-timeout 15` 覆盖 C.2 里 5s 即可到达 UE4 fatal 的窗口，采集 `0x1047862bc` 这一 Thread #10 `EXC_BAD_ACCESS (address=0x30)` 的 `faultingInstruction` / `instructionByteStream` / `imageOffset`，与 `.ips` 里同一 fault 的 `usedImage.base + imageOffset` 对齐；产出 `build/hok-007c-ngr-callsite-report.json`（字段结构复用 HOK-007A 的 `build/hok-007-ngr-callsite-report.json`）。
  2. `HOK-007C.2`：基于 C.1 的离线映射结论，判断这个 fault 是"`0x10e2146f8` reader chain 的下游"还是"与它无关的独立 `ldr x8, #imm8` null-deref"；前者可接着评估 HOK-007B 风格的"单 4-byte 可逆 patch"是否还能继续把 app 压到更下游，后者直接退出本条并完全切到 HOK-010。
  3. `HOK-010.A`：把 `PlaySettings.disableForMinimalStartupCompat(...)` 对 `rootWorkDir` 的强制关闭改成 "per-bundle override = 由 host 侧 `PlayApp.effectiveLaunchEnvironment` 显式承担 cwd 设置，再放开 `rootWorkDir`"；先在离线层改 PlayTools，单元测试覆盖 `disableForMinimalStartupCompat` 对 `com.tencent.ngr` 的返回值变化。
  4. `HOK-010.B`：用 HOK-007C.1/C.2 的 callsite 报告与 HOK-010.A 改动同时部署，live 复测 `QtsFileSystem Create failed.` 是否消失；若消失，主线收束到"候选 E + HOK-010.A"作为长期方案，HOK-007C / HOK-012 全部保持 DEFER。
  5. 若 HOK-007C.1 或 HOK-010.A 的 live 证据**重新指向 `0x10e2146f8`**（比如 HOK-007C.1 发现 fault 实际上也是 `ldr x0, [0x10e2146f8]` 的别名路径），则 HOK-012 从 DEFER 复活，但下一轮不再重复 C.3 的 writer-entry bp，而是直接给 `launch_app_with_lldb` 扩展 `postLaunchInjectionLibrary` 字段，让 PlayTools 在 `_dyld_start` 附近 dlopen 一份带 `__attribute__((constructor))` 的辅助 dylib 提前写好该 slot。
...
| HOK-012-C.3 | DONE | deferred-install watchpoint：`LLDBRunOptions.deferWatchpointInstall` 贯穿 `LaunchService` / `runLLDBHeadless` / `launch_app_with_lldb` MCP schema / `hok006_ngr_lldb_runner.py --defer-watchpoint-install + --writer-address`（默认 `0x103a29b7c`）四层，Python runner 在 defer 模式下自动注入 `breakpoint set --address 0x103a29b7c -C "watchpoint set expression -s 8 -- 0x10e2146f8" -C "continue" --auto-continue true --one-shot true` 到 `preRunCommands`。C.3 在 `--lldb-timeout 5` 与 `--lldb-timeout 20` 两档 live 均 0 hit + bp 也未命中（transcript 有 `Breakpoint 1: address = 0x0000000103a29b7c` 但无 `Breakpoint 1.*hit` 行），证明 writer 函数 `0x103a29b7c` 在 app 启动 20s 内（已跨过 PlayTools compat 序列、GCloudCore / PluginManager / CAppLifecycleLaunch C++ ctor 与 `SceneExtension hook_frame` 循环）**从未被调用**；这证伪了 HOK-011 "slot writer 必经 `0x103a29b7c`" 的静态链。`LaunchServiceTests` 新增 2 个用例（`testLLDBRunOptionsDeferWatchpointInstallIsOrthogonalToWatchMode`、`testLaunchWithLLDBForwardsDeferWatchpointInstallToRunner`），26 个测试全过；`BuildScripts/build_and_install.sh` 构建 + 安装 + ad-hoc 签名成功 | `LocalDocs/HOKCrash/HOK-012-C-watchpoint-live-trace.md`（deferred-install 证据口径 + CLI 最小参照） |
| HOK-012 | DEFERRED | `0x10e2146f8` 的 writer 追查已走到本仓内可控手段的边缘（HOK-011 静态 + HOK-012-A/B/C live 三轮），继续追需要引入"PlayTools 内 dlopen 辅助 dylib 预写 slot"这种跨模块改动；候选 E 仍 apply 且足以把 app 压到 `QtsFileSystem` 阶段，主线从 HOK-012 整体切出；HOK-012-A/B/C 就位的自动化栈保持在位，作为任何新 `__common` slot 追查的默认工具 | `LocalDocs/HOKCrash/HOK-012-C-watchpoint-live-trace.md`、`LocalDocs/HOKCrash/HOK-011-静态初始化链分析.md` |
| HOK-010 | TODO | 把 `rootWorkDir` 从 `PlaySettings.disableForMinimalStartupCompat(...)` 摘除以消除 `QtsFileSystem Create Failed!!`（C.2 已 live 复现该 UE4 fatal 与 Thread #10 `0x1047862bc` secondary fault）。HOK-012 已整体 DEFER，主线之一切回本条；拆成 HOK-010.A（PlaySettings 改动 + host 侧 cwd override + 单测）与 HOK-010.B（live 复测 fatal 是否消失） | 待建 |
| HOK-007C | TODO | 对 C.2 里 Thread #10 `0x1047862bc` / `address=0x30` / `ldr w9, [x8, #0x30]` 做 HOK-007A 风格的离线 callsite 一致性映射（`instructionByteStream` + `.ips imageOffset` 交叉验证），并视结果决定是否补一个 HOK-007B 风格的最小可逆 patch。HOK-012 DEFER 后升级为与 HOK-010 并行的第二条主线 | `LocalDocs/HOKCrash/HOK-007-二进制意图分析与callsite映射.md` |
...
- HOK-012-C.3 live 证明："**backtrace 里出现 writer 函数内偏移** ≠ **writer 函数最近被调用**"。backtrace 是瞬时栈帧状态，`frame #4: 0x103a29fb0`（`0x103a29b7c + 0x434`）完全可能是该线程 pthread start 后进入的常驻函数。要验证 writer 是否真的"最近被调用"，必须在 writer 入口加 bp（deferred-install 模式下自动注入），在同等时间窗内观察 `Breakpoint N.*hit` 是否出现。本工程里加到 `0x103a29b7c` 的 bp 在 20s 窗口内**始终未命中**，这才是"slot 不是被该函数 first-initialize 的"最终 live 证据。
