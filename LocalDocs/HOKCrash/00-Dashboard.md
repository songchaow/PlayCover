...
- **当前结论**：`HOK-004`、`HOK-005A`–`HOK-005D`、`HOK-006`、`HOK-007A`、`HOK-007B`、`HOK-011`、`HOK-012-A`、`HOK-012-B`、`HOK-012-C.1`（legacy baseline）、`HOK-012-C.2`（pre-run watchpoint）、`HOK-012-C.3-a`（deferred-install watchpoint）、`HOK-012-C.3-b.0`（对话框检测 + 残留进程强杀 + 污染 gate）、`HOK-012-C.3-b.1`（SIGABRT 拦截 + abort-stop handler 四层自动化）、`HOK-012-C.3-b.2`（`runLLDBHeadless` 收尾窗口参数化为 `teardownTimeoutSeconds`）、`HOK-012-C.3-b.3`（`0x10e2146f8` 未污染 live 证据采集）、`HOK-013`（PlayTools 侧 stub-based slot preheat）均已完成。**`com.tencent.ngr` 启动期阻塞窗口的结构**：`NSAlert` 以 sheet modal 形式 attach 到 parent window，order 路径是 `-[NSAlert beginSheetModalForWindow:completionHandler:]_block_invoke` → `-[NSWindow _beginWindowBlockingModalSessionForSheet:service:completionHandler:isCritical:]` → `-[NSApplication _orderFrontModalWindow:relativeToWindow:]` → `-[NSWindow(NSSheets) _orderFrontRelativeToWindow:]` → `-[NSWindow orderFront:]`（`windowLayer=0` 是 sheet 特征）；sheet 一旦 order front，UI 线程进入 sheet modal session，其他线程继续跑但永不前进，所有 LLDB 读数变为冻结态快照。关键拦截 bp 是 `-[NSWindow orderFront:]`，`-[NSApplication runModalForWindow:]` 与 `CGSOrderWindow`（→ `SLSOrderWindow`）作为冷备覆盖非 sheet 的 modal 路径。**HOK-013 的 live 结论**：PlayTools 在其 `__attribute__((constructor))` 最早时刻（早于 NGR 主 image 的 `__init_offsets`）把一个 PlayTools 持有的 stub object 地址直接写入 `0x10e2146f8`，reader 原 faulting `ldr x8, [x19]` 读到 stub vtable、`ldr x8, [x8, #0x10]; blr x8` 命中 `pt_ngr_stub_vfunc_noop`（`mov x0, #0; ret`）安全返回；**候选 E revert 后 app 不再在 `0x10480df08` 崩**（run2: `lldbStopObserved=False`，无新 `NGR-*.ips`）；**候选 E 仍 apply 的 run1 里 slot 在 HOK-013 primed `0x115d7cfc0` 之后被 NGR 自己的真实 writer 覆盖为 `0x10e23f200`**，证明 iOS-equivalent 的 writer 路径确实存在、只是被候选 E 的 reader 绕过掩盖了；三轮验证全过（run1 overallPass=True + `SceneExtension hook_frame` 主循环就绪、run2 无 fault、run3 bundle-scoped gate 仅 `com.tencent.ngr` 生效）。原 HOK-011 报告把 writer 函数入口标成 `0x103a29b7c`，HOK-013 在 accessor 调用尝试中发现真正的 writer 入口是 `0x103a29c60`（`0x103a29b7c` 是相邻的另一个 Logger dispatch wrapper），这不影响"writer 不可达 NGR 自身 init_offsets"的结论，但解释了为何"调 accessor 而非写 stub"方案会在 writer 内部 `0x10481d7a8` 因 `x0=0x1` 崩掉——writer 透传 caller 的 x0 给 deep-callee，PlayTools constructor 窗口里 x0 无法提供合法 category pointer。`LaunchService.runLLDBHeadless` 的 abort-stop handler 两项固定规则：(A) slot 读必须显式 `memory read -fx -s 8 -c 1 <addr>`（不带 format 会与 LLDB 默认 byte-format 冲突、slot 值被静默丢弃）；(B) 尾部固定 `kill\nquit`（stop 现场已把证据采完，`continue` 会让 app 跑到 `-[NSWindow orderFront:]` 把 sheet 订上屏幕污染 b.0 gate）。`LaunchServiceTests` 28 个测试全过，`FORCE_PLAYTOOLS_REBUILD=1 ./BuildScripts/sync_playtools_xcframework.sh` + `./BuildScripts/build_and_install.sh Release` + ad-hoc 重签名成功。
- **当前已知事实**：
  ...
  - HOK-013 已落地：`Carthage/Checkouts/PlayTools/PlayTools/PlayLoader.m` 新增 `pt_ngr_stub_vfunc_noop`（naked `mov x0, #0; ret`）+ `pt_ngr_stub_vtable[8]` + `pt_ngr_stub_object`（16-byte aligned，vtable ptr + 56 字节 padding）+ `pt_ngr_preheat_slot_once()`；constructor 第一行调用，`dispatch_once` 保证只写一次。Bundle gate 用 `CFBundleGetIdentifier(CFBundleGetMainBundle())` 字符串比较 `com.tencent.ngr`，早于 PlaySettings 初始化。Slide 通过 `_dyld_image_count` / `_dyld_get_image_header` 找 `MH_EXECUTE` + 解析 `LC_SEGMENT_64 __TEXT.vmaddr` 计算（`slide=(uintptr_t)mh - unslidTextVMAddr`）；若 unslid `__TEXT.vmaddr != 0x100000000` 直接放弃 preheat 走候选 E 兜底。`PlayCover.swift` 新增 `@objc static func recordHOK013PreheatDiagnostic(details:)` 把 C 侧 details dict 落到 `launch-events.jsonl` 的 `hok013_ngr_slot_preheat` 事件。Live 验证（`build/hok-013-run1-report.json` + `build/hok-013-run2-baseline.json` + `launch-events.jsonl`）：write 事件 `status=primed stubObject=slotAfter=0x115d7cfc0`；run1 候选 E apply 下 slot 在 primed 之后被 NGR 自己覆盖为 `0x10e23f200`（真 Logger singleton）、app 跑到 `SceneExtension hook_frame` 主循环；run2 候选 E revert 下 `lldbStopObserved=False`、无新 `NGR-*.ips`；bundle-scoped gate 仅在 `com.tencent.ngr` 的 `launch-events.jsonl` 出现 HOK-013 事件。候选 E 当前保持 apply 作为安全网；revert 已被 live 证明安全，实际 revert 时机由运营决策。
  - 结构化证据：`build/hok-007-ngr-callsite-report.json`、`build/hok-007b-ngr-patch-report.json`、`build/hok-011-ngr-common-init-chain-report.json`、`build/hok-011-analysis-notes.json`、`build/hok-012-ngr-watchpoint-report.json`、`build/hok-012-ngr-dyld-initializers.log`、`build/hok-012-c-3-b-0-dialog-detection-evidence.json`、`build/hok-012-c-3-b-3-report.json`（b.3 未污染 live：slot=`0x0`、writer bp hit=0）、`build/hok-012-c-3-b-3-dyld.log`、`build/hok-013-run1-report.json`（HOK-013 候选 E apply 下 `overallPass=True`、slot 被覆写为真 Logger singleton）、`build/hok-013-run2-baseline.json`（HOK-013 候选 E revert 下 `lldbStopObserved=False` 无 crash）、`build/hok-013-run1-dyld.log`（均在 `.gitignore` 的 `build/` 下，本地留存）。
- **当前主线**：无硬主线。下一步可在以下方向挑选：(1) 把候选 E revert 变成长期状态（需运营决策，技术上 HOK-013 已兜住）；(2) 重启 `HOK-010`（摘除 `rootWorkDir` 强制关闭，让 `QtsFileSystem Create Failed!!` 能被正面解决）；(3) 排查 run1 里 slot 被 NGR 覆写为 `0x10e23f200` 的 writer 到底来自哪个 embedded framework initializer（用 HOK-012-C watchpoint pre-run 模式现在可以在干净条件下命中）。
- **当前卡点**：无。
- **下一步默认规划**：
  1. （可选）`HOK-010` 重启：把 `rootWorkDir` 从 `PlaySettings.disableForMinimalStartupCompat(...)` 摘除，看 `QtsFileSystem Create Failed!!` 在 HOK-013 兜底下能否被正面解决。预估需要评估 cwd 语义对其它 app 的影响；子任务先在 HOK-010 子文档里分析。
  2. （可选）`HOK-012-C.3-b.6`：复用 HOK-012-B 的 pre-run watchpoint 在 HOK-013 下做一次干净的 writer 捕获——此时 slot 在 preheat 后非 0，watchpoint `modified` 事件会明确指向那位真正覆写 slot 的 initializer；结合 `build/hok-013-run1-dyld.log` 的 ObjC 重复类警告 + `[GPM] cpp constructor call` 可以把 H1/H2/H3 假设进一步收窄。
  3. `HOK-012-C.3-b.4` / `b.5`（保留）：原"slot=0 分流"的兜底路径；当前 slot 已被 HOK-013 预热为非 0，这两条保留路径在当前条件下无触发动机。
