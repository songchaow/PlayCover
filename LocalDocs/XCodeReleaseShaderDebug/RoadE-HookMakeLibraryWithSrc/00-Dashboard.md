...
- **E-006**：`LLVMDisassembler` 的 runtime→host `llvm-dis` bridge 已落地，并已通过 `FORCE_PLAYTOOLS_REBUILD=1 ./BuildScripts/sync_playtools_xcframework.sh`、`./BuildScripts/build_gui.sh`、`./BuildScripts/test_mcp.sh RegistrationListenerHostCommandTests` 验证编译与桥接链路。此前 host bridge 版本的 **5 轮受控 live 复测**仍然 **5/5 稳定复现** `session: ready -> disconnected`，并持续伴随 `source recompile failed: program_source:15:16: error: expected unqualified-id`；说明 blocker 已前移到 IR→MSL 产物 / fallback / 运行时稳定性。本轮先完成了 `E-006a2a`：在 `makeLibrary(source:)` 前对聚合 MSL 做 preflight，遇到明显残留的 LLVM token（如 `<N x T>` / `ptr` / `addrspace` / `%...` / 原始 `i32`）时直接 fallback 原始 library，不再盲目触发重编译；若 preflight 拒绝或 compile failure，都会把 `.metal` / `.txt` 诊断样本落到 `~/Library/Containers/io.playcover.PlayCover/ShaderSourceDiagnostics/<bundleId>/`，compile failure 还会额外记录 `program_source` 行列与上下文。代码层面已通过 `FORCE_PLAYTOOLS_REBUILD=1 ./BuildScripts/sync_playtools_xcframework.sh` 与 `./BuildScripts/build_gui.sh` 验证；下一步应基于这版构建做 live 对照复测，优先判断“跳过明显非法重编译”后是否仍稳定崩溃
