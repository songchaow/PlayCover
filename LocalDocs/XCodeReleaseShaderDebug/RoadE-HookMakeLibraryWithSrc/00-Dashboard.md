...
- **E-006**：`LLVMDisassembler` 的 runtime→host `llvm-dis` bridge、`makeLibrary(source:)` 前的 MSL preflight guard，以及 `E-006a2c1` 的 `Toucher.touchcam` 空值防护均已落地，并已通过 `FORCE_PLAYTOOLS_REBUILD=1 ./BuildScripts/sync_playtools_xcframework.sh`、`./BuildScripts/build_gui.sh`、`./BuildScripts/test_mcp.sh RegistrationListenerHostCommandTests` 验证构建与桥接链路。2026-04-03 基于这版构建完成了 `E-006a2c2` 的 **关闭 `keymapping` 后 2 轮受控 live 复测**：按 `PLAYCOVER_INSTALL_MODE=user FORCE_PLAYTOOLS_REBUILD=1 ./BuildScripts/build_and_install.sh` 重装 GUI，对原神设置 `keymapping=false`，并从 `pkill Yuanshen` 干净状态执行 `remove_playtools` / `inject_playtools` / `launch_app` / `create_session`。第 1 轮 `session` 在 `00:47:37 +0800` ready 后，前 10 秒仍保持 ready，约 **42 秒**后 app 退出并新增 `Yuanshen-2026-04-03-004819.ips`；第 2 轮 `session` 在 `00:50:00 +0800` ready 后约 **10 秒**即变为 `disconnected`，并新增 `Yuanshen-2026-04-03-005012.ips`。两轮新 crash 均不再落在 `playcover.toucher`，而是统一回到 `UnityGfxDeviceWorker -> objc_release -> objc_autoreleasePoolPop` 的 `EXC_BAD_ACCESS / SIGSEGV`；与此同时，`~/Library/Containers/io.playcover.PlayCover/ShaderSourceDiagnostics/com.miHoYo.Yuanshen/` 新增了多份 `preflight_rejected` / `compile_failed` 样本，最新 compile failure 仍为 `program_source:14:16: expected unqualified-id`，坏行示例是 `fragment float4{ <4 xlatMtlMain(...)`。本轮已继续完成 `E-006a2d1` 的**离线修复**：`IRToMSLConverter` 现会先把 fragment/vertex 的 packed aggregate 返回值（如最小样本 `define <{ <4 x float> }> @test_fragment_packed(...)` 与 live 样本里的 `<{ <4 x float>, i8 }>`）当作 aggregate，而不是误判成 `<N x T>` 向量；同时在 metadata 过滤掉唯一默认 builtin 参数时，会把 IR `%0` 接回默认 MSL builtin 名（如 fragment 的 `position`）。已新增 `test_fragment_packed_return.metal/.ll`；修复后离线转换结果恢复为 `fragment float4 test_fragment_packed(float4 position [[position]])`，并通过 `xcrun --sdk macosx metal -c` 与 `FORCE_PLAYTOOLS_REBUILD=1 ./BuildScripts/sync_playtools_xcframework.sh` 验证。这说明已知 compile failure 中的**坏函数签名**已被离线堵住，当前剩余 blocker 已进一步收敛到 **`%0.field*`、`<4 x float>`、`fadd/fmul fast` 等 vector / SSA / fast-math 泄漏**，以及其伴随的 worker 线程崩溃。
...
|  | runtime→host 的 `llvm-dis` bridge、聚合 MSL preflight guard，以及 keymapping 隔离 live 复测均已完成；本轮又继续落地 `E-006a2d1` 的 fragment packed-return 离线修复，并通过 toolchain + PlayTools 构建验证。下一步转向 `E-006a2d2`，继续清 `%0.field*`、`<4 x float>`、`fadd/fmul fast` 等 preflight 泄漏，再做下一轮 live 截帧复测 | | |
...
|  | 已拆成“`E-006a2d1` 先修 fragment packed-return / 默认 builtin 参数映射”“`E-006a2d2` 再继续清 `%0.field*`、`<4 x float>`、`fadd/fmul fast` 等 LLVM token 泄漏”两步，避免把坏函数签名和剩余 lowering 泄漏继续混在一起 | | |
...
| E-006a2d2 | ↳ 继续修 vector / SSA / fast-math 泄漏 | TODO | |
|  | 继续结合 `2026-04-02T16_50_11Z_*` 等 preflight 样本，优先清 `%0.field*`、`<4 x float>`、`fadd/fmul fast` 等 LLVM token 泄漏，再做下一轮 live 截帧复测 | | |
