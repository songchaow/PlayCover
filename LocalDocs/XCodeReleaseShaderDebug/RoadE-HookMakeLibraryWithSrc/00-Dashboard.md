...
- **当前最该做的事**：`E-006d8` 的第一轮 run matrix 已经够用，下一步不再是继续机械补 run，也不是回头扩 lowering。2026-04-05 本轮已按标准路径完成 fresh `replacement-on-run5`：`./BuildScripts/build_and_install.sh` → `remove_playtools / inject_playtools / launch_app / create_session(timeout=20)`，结果成功拿到 `session=ready`；`RuntimeLaunchDiagnostics/<bundleId>/launch-events.jsonl` 也明确命中了 `playcover_launch_enter`、`playcover_bridge_listener_start_requested`、`bridge_listener_command_listener_ready`、`bridge_registration_attempt_started`、`bridge_registration_established` 与 `playcover_launch_complete`，说明 fresh 注入后的 runtime **确实已经进入 `PlayCover.launch()` / `BridgeListener.start()` 并建立 registration channel**，旧的“可能根本没进入口”怀疑可以收束。随后 `get_capture_status` 返回 `available=true` / `supportsGPUTrace=true`，改用默认输出路径后已成功导出新的 `replacement-on-run5` `.gputrace` 并并回标准快照；当前真正的 blocker 进一步收窄为：1）同轮 replacement 仍只**部分命中**，manifest 历史/当前快照里仍可见一批 `reasonCode=exception`、`detail="Failed to launch llvm-dis: Operation not permitted"`；2）`capture_metal_frame` 在**自定义工作区输出路径**上仍会遇到权限失败，只能先走目标 app 容器默认 `Captures/` 再纳入快照；3）新 `.gputrace` 目前只检出 `11` 个源码文件中的 `2` 个合法 MSL，说明“live 链路已恢复”不等于“trace 可见性 / replacement 覆盖已稳定充分”。在这三条线收敛前，不应回头把主 blocker 重新定性为 launch/bootstrap 问题，也不应把日常 gate 推回人工协助。
...
| 当前 `E-006d8` 第一层矩阵结论（2026-04-05） | `off1/off2/on1/on2/on3/on4/on5` 七轮快照继续证明：**同模式输入稳定**、共享 `moduleKey` 为 **91/91**、单模块 `.bc/.ll/.metal` 本体未出现语义级随机漂移；将 fresh `replacement-on-run5` 纳入后，`build/e006d-run-matrix.json` 最新摘要为：`groups.off.pairSummary.allPairsInputStable=true`、`groups.on.pairSummary.allPairsInputStable=true`、`groups.on.pairSummary.allSnapshotStable=true`、`crossMode.offVsOnPairSummary.allPairsDifferent=false`、`differentPairCount=8` |
| 当前尚未证明的部分（2026-04-05） | 现阶段**仍未形成“跨模式稳定不同”的 corpus / trace 级证据**：虽然 fresh `replacement-on-run5` 已重新拿到 `.gputrace`，且 `RuntimeLaunchDiagnostics` / `session` / `capture_status` 都证明 live 链路已恢复，但 `build/e006d-run-matrix.json` 仍给出 `crossMode.offVsOnPairSummary.allPairsDifferent=false`；同时新 trace 仅 `源码文件=11`、`合法 MSL=2`、覆盖率 `1.2%`，因此还不能把结论提前收敛为“替换开启后一定稳定引入了最终差异” |
| 当前 replacement 证据基线（2026-04-05） | 标准 `BuildScripts/build_and_install.sh` + fresh `replacement-on-run5` 已进一步把 live 证据闭环补齐：`create_session(timeout=20)` 成功得到 `session=ready`，`RuntimeLaunchDiagnostics` 命中完整 launch / listener / registration 链路，`get_capture_status` 返回 `available=true` / `supportsGPUTrace=true`，并成功导出新的 `.gputrace`；该轮快照摘要为 `manifestLines=1909`、`modules=91`、`replacements=127`、`diagnostics=18`、`gputraceMSL=2`。这说明当前已不再是“fresh on-run 无法注册或无法截帧”，但 replacement 覆盖与 trace 可见性仍未完全稳定 |
| 当前 blocker 与下一步（2026-04-05） | 真正的 blocker 已从“fresh 注入后 runtime 可能根本没进 `PlayCover.launch()`”前移为三件事：1）同轮 replacement 仍只部分命中，manifest 历史/当前快照里仍可见 `Failed to launch llvm-dis: Operation not permitted`；2）`capture_metal_frame` 使用自定义工作区 `output_path` 会因权限失败，只能先导出到目标 app 容器默认 `Captures/` 后再纳入标准快照；3）`replacement-on-run5` 新 `.gputrace` 只有 `2/11` 个合法 MSL，说明 live/capture 链路虽已恢复，但最终 trace 侧源码覆盖仍偏低。下一步应优先沿这三条自动化路径继续收敛，而不是再把重心放回“入口有没有跑到”或重新引入人工 gate |
...
|  | `RuntimeLaunchDiagnostics/<bundleId>/launch-events.jsonl` 与 `Scripts/runtime_launch_diagnostics_summary.py` 已在 fresh `replacement-on-run5` 中完成实战验证：最新 `processLaunchId` 已命中 `playcover_launch_enter`、`bridge_listener_command_listener_ready`、`bridge_registration_established` 与 `playcover_launch_complete`，并且 `create_session(timeout=20)` / `get_capture_status` / 新 `.gputrace` 导出都已成功。当前这条线的价值已从“确认入口有没有跑到”前移为“证明 launch/session/capture 主链已恢复”；下一步应把重心转到并行收敛 **`llvm-dis` `Operation not permitted` 失败样本**、**自定义 `capture` 输出路径权限** 与 **trace 侧合法 MSL 覆盖偏低** 这三件事，而不是继续停留在 unified log 猜测或重新引入人工 gate。 | | |
