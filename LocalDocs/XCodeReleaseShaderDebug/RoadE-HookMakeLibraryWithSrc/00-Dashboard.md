...
| `恋与深空` 三开关启动兼容性 blocker（2026-04-07） | 最近几轮提交已依次完成 **failure surfaces 关联 → hotspot workflow 固化 → late replacement failures 捕获**。最新结论：`B/C` 可在 launch 后继续存活，`D/E` 会在 `playcover_launch_complete` 之后约 7~8 秒内 late crash。**本轮已把 `case E` 快照刷新为按 `launchSettings` 过滤的 case-specific 摘要**：`matchedRuns=2`、`ignoredRuns=23`、`latestLastEvent=replacement_compile_failed`。当前 startup replacement 的命中面已收敛到：历史 `cacheKey=6BECB97B0B4BCBFD_7123`（现已 bypass），以及新的 `cacheKey=791A306ED1B6648B_4577`（`expected expression`）与 `cacheKey=F474C54E8C5214F4_4689`（`mtl_BaseVertex` 未声明）；其中已完成 manifest 关联的最小 failure surface 为 `F474C54E8C5214F4_4689 -> moduleKey=bbb32dc1261c6c5bb2e8d05d0983d7938cfc68cd9556fed2faed2d0852e04854`。**本轮又补齐了 runtime diagnostics 直接携带 `moduleKeys/moduleKeyCount`，且在 manifest 缺口时可回退生成 `runtime_event` 证据的 failure surface，避免 `E-006g3` 被“看得到 compile_failed、收不到最小模块面”的缺口卡住。** 因此**当前默认任务仍不是继续补脚本，而是基于这组 case-specific hotspot / failure surface 继续推进 `E-006g3` 的最小旁路面收缩。** |
