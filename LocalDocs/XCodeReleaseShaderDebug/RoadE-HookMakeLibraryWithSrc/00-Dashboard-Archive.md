...
| 2026-04-06，`E-006d8` fresh `replacement-on-run8` / `on-run9` | 这两轮是 capture bridge reachability 关键证据：直接对 ready session 执行 `get_capture_status` / `capture_metal_frame` 可立即成功，但同轮 `create_session` 仍报 bridge not reachable；自定义 `output_path` 在 app 默认容器 `Captures/` 内可成功。**blocker #1 从"capture/status 命令整体不可达"收窄到 `create_session` reachability probe / ready-session 复用判定分歧。** | blocker #1 已基本收敛 |
| 2026-04-06，`E-006d8` fresh `replacement-on-run10` + blocker #1 live 复测 | `create_session(bundleId)` 首次即返回 ready session，容器内 `capture_metal_frame` 成功并经 `finalize-run --latest-gputrace` 固化。短暂 `list_sessions` 空窗属 registry 可见性抖动。**blocker #1 基本收敛**，主排查面转向 trace 合法 MSL 覆盖偏低 | blocker #1 基本收敛，主瓶颈转向 trace 合法 MSL 覆盖偏低 |
